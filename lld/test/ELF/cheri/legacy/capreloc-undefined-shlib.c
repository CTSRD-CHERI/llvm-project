...
// SHLIB-RELOCS-NEXT:  }
// SHLIB-RELOCS-NEXT:]
// RUN: llvm-objdump -C -s -r -t -h %t.so
// RUN: llvm-objdump -C -r -s -t -h %t.so | FileCheck %s
...
// CHECK:  .data         00000010 0000000000010000 DATA
...
// READOBJ-LABEL:Section (8) .rela__cap_relocs {
