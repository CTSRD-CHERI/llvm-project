...
// RUN: ld.lld --no-relative-cap-relocs -process-cap-relocs %t.o -static -o %t-static.exe -verbose 2>&1 | FileCheck -check-prefixes LINKING-EXE %s
...
// RUN: ld.lld --no-relative-cap-relocs -process-cap-relocs -pie -Bdynamic %t.o -o %t-dynamic.exe -verbose 2>&1 | FileCheck -check-prefixes LINKING-DYNAMIC %s
...
// RUN: ld.lld --no-relative-cap-relocs -process-cap-relocs %t.o -shared -o %t.so -verbose 2>&1 | FileCheck -check-prefixes LINKING-DYNAMIC %s
...
// OBJ-CAPRELOCS: 0x0000000000000000      Base: __error_selector (0x0000000000000000)     Offset: 0x0000000000000000      Length: 0x0000000000000000      Permissions: 0x00000000{{$}}
