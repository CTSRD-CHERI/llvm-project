...
// RUN: llvm-objdump -h -r -t  --cap-relocs %t-direct-static.exe | FileCheck -check-prefixes DUMP-EXE,STATIC %S/simple-cap-reloc-common.check
...
// RUN: llvm-objdump -h -r -t  --cap-relocs %t-direct-dynamic.exe | FileCheck -check-prefixes DUMP-EXE,DYNAMIC %S/simple-cap-reloc-common.check
