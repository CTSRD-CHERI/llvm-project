...
// RUN: ld.lld -shared %t-shlib.o -o %t2.o
// RUN: llvm-objdump --syms  %t2.o | FileCheck %s -check-prefix OTHER_FILE-SYMS
// OTHER_FILE-SYMS: 0000000000010000 g F .text 00000034 check_if_default
...
// RUN: llvm-readobj --dyn-symbols --dyn-relocations --cap-relocs --cap-table %t.so | FileCheck %s -check-prefixes CHECK,CHECK-SHLIB,CHECK-NO-VERSION-SCRIPT
// And for -pie:
// RUN: ld.lld -pie %t.o %t2.o -o %t-pie.exe  --verbose-cap-relocs
// RUN: llvm-readobj --dyn-symbols --dyn-relocations --cap-relocs --cap-table %t-pie.exe | FileCheck %s -check-prefixes CHECK,CHECK-NO-VERSION-SCRIPT
...
// RUN: llvm-readobj --dyn-symbols --dyn-relocations --cap-relocs --cap-table %t.exe | FileCheck %s -check-prefixes CHECK,CHECK-NO-VERSION-SCRIPT

// CHECK-LABEL: Dynamic Relocations {
// With the version script default_callback comes first
// CHECK-VERSION-SCRIPT-NEXT:   R_MIPS_CHERI_CAPABILITY/R_MIPS_NONE/R_MIPS_NONE __cheri_fnptr_default_callback 0x0
// CHECK-VERSION-SCRIPT-NEXT:   R_MIPS_CHERI_CAPABILITY/R_MIPS_NONE/R_MIPS_NONE __cheri_fnptr_default_callback 0x0
...
// CHECK-NEXT:     R_MIPS_CHERI_CAPABILITY_CALL/R_MIPS_NONE/R_MIPS_NONE check_if_default 0x0
// CHECK-NEXT:  }
...
// CHECK-NEXT:   }
// With the version script we also need to add a fake symbol for default_callback
// CHECK-VERSION-SCRIPT-NEXT:   Symbol {
// CHECK-VERSION-SCRIPT-NEXT:     Name: __cheri_fnptr_default_callback ({{.+}})
// CHECK-VERSION-SCRIPT-NEXT:     Value: 0x10000
// CHECK-VERSION-SCRIPT-NEXT:     Size: 12
// CHECK-VERSION-SCRIPT-NEXT:     Binding: Local (0x0)
// CHECK-VERSION-SCRIPT-NEXT:     Type: Function (0x2)
// CHECK-VERSION-SCRIPT-NEXT:     Other [ (0x2)
// CHECK-VERSION-SCRIPT-NEXT:       STV_HIDDEN (0x2)
// CHECK-VERSION-SCRIPT-NEXT:     ]
// CHECK-VERSION-SCRIPT-NEXT:     Section: .text (0x
// CHECK-VERSION-SCRIPT-NEXT:   }
// CHECK-NEXT:   Symbol {
...
// CHECK-NEXT:   }
// CHECK-NEXT:  Symbol {
// CHECK-NEXT:    Name: check_if_default (
// CHECK-NEXT:    Value: 0x0
// CHECK-NEXT:    Size: 0
...
// CHECK-NEXT:   0x10 check_if_default@CAPTABLE R_MIPS_CHERI_CAPABILITY_CALL/R_MIPS_NONE/R_MIPS_NONE against check_if_default
...
// RUN: llvm-readobj --dyn-symbols --dyn-relocations --cap-relocs --cap-table %t-static.exe
...
// RUN: llvm-readobj --dyn-symbols --dyn-relocations --cap-relocs --cap-table %t-version-script.so | FileCheck %s -check-prefixes CHECK,CHECK-VERSION-SCRIPT

// FIXME: this should also be a runtime test in rtld

typedef int (*func_ptr)(void);
_Bool check_if_default(func_ptr ptr);
