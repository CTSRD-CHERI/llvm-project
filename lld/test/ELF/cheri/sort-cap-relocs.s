...
// RUN: ld.lld %t1.o %t2.o -static -o %t.exe --script=%t.script --no-sort-cap-relocs
...
// RUN: ld.lld %t2.o %t1.o -static -o %t.exe --script=%t.script --no-sort-cap-relocs
...
// RUN: ld.lld %t1.o %t2.o -static -o %t.exe --script=%t.script --sort-cap-relocs
// RUN: llvm-objdump -s --section=__cap_relocs -C -h %t.exe | FileCheck %s -check-prefix SORTED
// RUN: ld.lld %t2.o %t1.o -static -o %t.exe --script=%t.script --sort-cap-relocs
// RUN: llvm-objdump -s --section=__cap_relocs -C -h %t.exe | FileCheck %s -check-prefix SORTED
...
// RUN: ld.lld %t1.o %t2.o -shared -o %t.so --script=%t.script --no-sort-cap-relocs
...
// SHLIB-UNSORTED1-NEXT: 0x0000000000010040	Base:  (0x0000000000000000)	Offset: 0x0000000000000000	Length: 0x0000000000000004	Permissions: 0x8000000000000000 (Function)
// SHLIB-UNSORTED1-NEXT: 0x0000000000020020	Base:  (0x0000000000000000)	Offset: 0x0000000000000000	Length: 0x0000000000000004	Permissions: 0x8000000000000000 (Function)
...
// SHLIB-UNSORTED1-NEXT:  20050 00000000 00000000 00000000 00000004  ................
...
// SHLIB-UNSORTED1-NEXT:  20080 00000000 00000004 80000000 00000000  ................
...
// RUN: ld.lld %t2.o %t1.o -shared -o %t.so --script=%t.script --no-sort-cap-relocs
...
// SHLIB-UNSORTED2-NEXT:  20050 00000000 00000000 00000000 00000004  ................
...
// SHLIB-UNSORTED2-NEXT:  20080 00000000 00000004 80000000 00000000  ................
...
// RUN: ld.lld %t1.o %t2.o -shared -o %t.so --script=%t.script --sort-cap-relocs
// RUN: llvm-objdump -s --section=__cap_relocs -C -h %t.so | FileCheck %s -check-prefix SHLIB-UNSORTED1
// RUN: llvm-readobj -r %t.so | FileCheck %s -check-prefix SHLIB-RELOCS-UNSORTED1
// RUN: ld.lld %t2.o %t1.o -shared -o %t.so --script=%t.script --sort-cap-relocs
// RUN: llvm-objdump -s --section=__cap_relocs -C -h %t.so | FileCheck %s -check-prefix SHLIB-UNSORTED2
// RUN: llvm-readobj -r %t.so | FileCheck %s -check-prefix SHLIB-RELOCS-UNSORTED2
