...
// RUNTODO: llvm-objdump -h -r -t -C %t-dynamic.exe | FileCheck -check-prefixes DUMP-EXE,DYNAMIC %s

// Look at shared libraries:
// RUN: ld.lld -process-cap-relocs %t.o -shared -o %t.so
// RUNTODO: llvm-readobj -r -s %t.so | FileCheck -check-prefixes SHLIB,SHLIB-INTEGRATED %s
// RUNTODO: llvm-objdump -C -t %t.so | FileCheck -check-prefixes DUMP-SHLIB,DUMP-SHLIB-INTERNAL %s
...
// PIE exe should have dynamic relocations and only the offset values
// DYNAMIC-NEXT: 0x0000000000010020      Base:  (0x0000000000000000)     Offset: 0x0000000000000000      Length: 0x0000000000000017      Permissions: 0x00000000
// DYNAMIC-NEXT: 0x0000000000010060      Base: <Unnamed symbol> (0x0000000000000006)     Offset: 0x0000000000000004      Length: 0x0000000000000017      Permissions: 0x00000000
// DYNAMIC-NEXT: 0x00000000000100a0      Base: <Unnamed symbol> (0x000000000000000d)     Offset: 0x0000000000000000      Length: 0x0000000000000017      Permissions: 0x00000000
// DYNAMIC-NEXT: 0x00000000000100e0      Base: <Unnamed symbol> (0x000000000000000d)     Offset: 0x0000000000000000      Length: 0x0000000000000017      Permissions: 0x00000000
// DYNAMIC-NEXT: 0x0000000000010120      Base: <Unnamed symbol> (0x000000000000000d)     Offset: 0x0000000000000001      Length: 0x0000000000000017      Permissions: 0x00000000
