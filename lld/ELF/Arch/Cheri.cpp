...
static uint64_t getTargetSize(const CheriCapRelocLocation &Location,
  ...
  if (TargetSize == 0 && !TargetSym->IsPreemptible) {
    bool WasWeakSym = !TargetSym->isLocal() && TargetSym->WasUndefWeak;
    if (WasWeakSym)  {
      // Symbols previously declared as weak can have size 0 (if they then resolve
      // to NULL)
      // For example __preinit_array_start, etc. are generated by the linker as ABS
      // symbols with value 0
      if (TargetSym->getVA(0) != 0) {
        StringRef Name = TargetSym->getName();
        bool IsSpecialEndSymbol = Name.startswith("__") &&
                                  Name.endswith("_end");
        if (Name == "end" || Name == "_end" || Name == "etext" ||
            Name == "_etext" || Name == "edata" || Name == "_edata")
          IsSpecialEndSymbol = true;
        // All previously weak symbols that resolve to an address should have
        // a size > 0. The only exception are e.g. __preinit_array_end, etc.
        // which should actually be zero size symbol since they are just markers
        // for the end of a section and not usable as a valid pointer
        if (!IsSpecialEndSymbol)
          error("Zero size weak symbol did not resolve to NULL but 0x" +
                utohexstr(TargetSym->getVA(0)) + ": " +
                lld::verboseToString<ELFT>(TargetSym, 0));
      }
      return 0;
    }
