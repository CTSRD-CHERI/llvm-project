...
void ARM::relocate(Compartment *c, uint8_t *loc, const Relocation &rel,
                   uint64_t val) const {
  switch (rel.type) {
  case R_ARM_ABS32:
  case R_ARM_BASE_PREL:
  ...
  case R_ARM_THM_CALL: {
    ...
    if (!rel.sym->isFunc() && !rel.sym->isInPlt(c) && isBlx == bit0Thumb)
      stateChangeWarning(loc, rel.type, *rel.sym);
    if (rel.sym->isFunc() || rel.sym->isInPlt(c) ? !bit0Thumb : isBlx) {
...
void ArmCmseSGSection::writeTo(uint8_t *buf) {
  for (ArmCmseSGVeneer *s : sgVeneers) {
    ...
    write16(p + 6, 0xb000);
    target->relocateNoSym(compartment, p + 4, R_ARM_THM_JUMP24,
                          s->acleSeSym->getVA() -
