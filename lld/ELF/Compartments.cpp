...
public:
    ...
    for (const auto &rule : rules) {
      ...
      if (!matches) {
        for (const auto &glob : rule.compartments) {
          ...
          if (symCompart && glob.match(symCompart->name)) {
            matches = true;
            break;
          }
        }
  ...
  bool hasMatch(const Compartment &c, const Compartment &target, Pred p) const {
    ...
    for (const auto &rule : rules) {
      // Check the subject.
      if (!rule.subject.isTrivialMatchAll()) {
        if (&c == defaultCompart || !rule.subject.match(c.name))
          continue;
      }
      ...
      for (const auto &glob : rule.compartments) {
        ...
        if (&target != defaultCompart && glob.match(target.name)) {
          matches = true;
          break;
        }
      }
