...
public:
  ...
  bool hasMatch(const Compartment &c, const Symbol &sym, Pred p) const {
    const Compartment *symCompart = sym.containingCompartment();
    ...
    for (const auto &rule : rules) {
      ...
      if (!rule.subject.isTrivialMatchAll()) {
        if (!rule.subject.match(c.isDefault() ? "" : c.name))
          continue;
      }
      ...
      if (!matches) {
        for (const auto &glob : rule.compartments) {
          ...
          if (symCompart &&
              glob.match(symCompart->isDefault() ? "" : symCompart->name)) {
            matches = true;
            break;
          }
        }
  ...
  bool hasMatch(const Compartment &c, const Compartment &target, Pred p) const {
    ...
    for (const auto &rule : rules) {
      // Check the subject.
      if (!rule.subject.isTrivialMatchAll()) {
        if (c.isDefault() || !rule.subject.match(c.name))
          continue;
      }
      matchedAnySubject = true;
      ...
      for (const auto &glob : rule.compartments) {
        ...
        if (!target.isDefault() && glob.match(target.name)) {
          matches = true;
          break;
        }
      }
...
void verifyExecSymbol(const Compartment &c, const InputSectionBase &sec,
  ...
  error("policy forbids executing " + toString(sym) + " from " +
        compartmentName(c) + getReferenceMessage(sec, sym, off));
}
...
void verifyAccessSymbol(const Compartment &c, const InputSectionBase &sec,
  ...
  error("policy forbids accessing " + toString(sym) + " from " +
        compartmentName(c) + getReferenceMessage(sec, sym, off));
}
...
bool canWriteSection(const Compartment &c, const InputSectionBase *isec) {
  return acls.empty() || acls.hasMatch(c, isec->getCompartment(), requireWrite);
}
