...
static void addTgotEntry(Compartment *c, Symbol &sym) {
  ...
  if (sym.isPreemptible) {
    c->relaTgot->addReloc({target->tgotRel, c->tgot.get(), off,
  ...
  if (sym.isUndefWeak())
    ...
    c->tgotCapRelocs->addReloc(*c->tgot, off, sym, 0, expr, type);
  else
    c->relaTgot->addReloc(DynamicReloc::AddendOnlyWithTargetVA, target->tgotRel,
