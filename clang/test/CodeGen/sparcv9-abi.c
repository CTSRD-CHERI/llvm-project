...
// CHECK-LABEL: define dso_local signext i32 @f_int_1(i32 signext %x)
int f_int_1(int x) { return x; }

// CHECK-LABEL: define dso_local zeroext i32 @f_int_2(i32 zeroext %x)
unsigned f_int_2(unsigned x) { return x; }

// CHECK-LABEL: define dso_local i64 @f_int_3(i64 %x)
long long f_int_3(long long x) { return x; }

// CHECK-LABEL: define dso_local signext i8 @f_int_4(i8 signext %x)
char f_int_4(char x) { return x; }

// CHECK-LABEL: define dso_local fp128 @f_ld(fp128 %x)
...
// CHECK-LABEL: define dso_local %struct.small @f_small(i32* %x.coerce0, i32* %x.coerce1)
...
// CHECK-LABEL: define dso_local %struct.medium @f_medium(%struct.medium* %x)
...
// CHECK-LABEL: define dso_local void @f_large(%struct.large* noalias sret %agg.result, %struct.large* %x)
...
// CHECK-LABEL: define dso_local i64 @f_reg(i64 %x.coerce)
...
// CHECK-LABEL: define dso_local inreg %struct.mixed @f_mixed(i32 inreg %x.coerce0, float inreg %x.coerce1)
...
// CHECK: define dso_local { i64, double } @f_mixed2(i64 %x.coerce0, double %x.coerce1)
...
// CHECK-LABEL: define dso_local i64 @f_tiny(i64 %x.coerce)
...
// CHECK-LABEL: define dso_local void @call_tiny()
...
// CHECK-LABEL: define dso_local signext i32 @f_variable(i8* %f, ...)
// CHECK: %ap = alloca i8*
