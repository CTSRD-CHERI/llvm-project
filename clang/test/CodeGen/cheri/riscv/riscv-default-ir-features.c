...
// RUN:   | FileCheck %s --check-prefixes=CHECK,RV32-NOCHERI '-DFEATURES=+32bit,+a,+c,+m,+relax' '-DSAME_FEATURES=-save-restore'
// RUN: %clang --target=riscv64 -S -emit-llvm %s -o - \
// RUN:   | FileCheck %s --check-prefixes=CHECK,RV64-NOCHERI '-DFEATURES=+64bit,+a,+c,+m,+relax' '-DSAME_FEATURES=-save-restore'
...
// RUN:    | FileCheck %s --check-prefixes=CHECK,RV64-XCHERI-PURECAP,NO-RVC '-DFEATURES=+64bit,+cap-mode,+relax,+xcheri'
...
// RUN:   | FileCheck %s --check-prefixes=CHECK,RV32-NOCHERI,NO-RVC '-DFEATURES=+32bit,+a,+c,+m,+relax'
// RUN: %clang --target=riscv64 -mno-xcheri-norvc -S -emit-llvm %s -o - \
// RUN:   | FileCheck %s --check-prefixes=CHECK,RV64-NOCHERI,NO-RVC '-DFEATURES=+64bit,+a,+c,+m,+relax'
