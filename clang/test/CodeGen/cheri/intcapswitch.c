...
int x(__intcap_t y)
{
  // CHECK-LABEL: define dso_local signext i32 @x(ptr addrspace(200) noundef
  // CHECK: [[SWITCH_VAR:%.+]] = call i64 @llvm.cheri.cap.address.get.i64(ptr addrspace(200) {{%.+}})
...
int y(void) {
  ...
  // DONTCHECK-OPT-LABEL: define dso_local noundef signext i32 @y()
...
int z_long(void) {
  // CHECK-OPT-LABEL: define dso_local noundef signext i32 @z_long()
  // CHECK-OPT:      [[SWITCHVAL:%.+]] = tail call i64 @llvm.cheri.cap.address.get.i64(ptr addrspace(200) nonnull @buf)
...
int z_intcap(void) {
  ...
  // CHECK-OPT-LABEL: define dso_local noundef signext i32 @z_intcap()
...
int z_fixed_offset(void) {
  ...
  // CHECK-OPT-LABEL: define dso_local noundef signext i32 @z_fixed_offset()
  // CHECK-OPT-NOT: ret i32 4
  // CHECK-OPT: [[CAP:%.+]] = tail call ptr addrspace(200) @llvm.cheri.cap.offset.set.i64(ptr addrspace(200) nonnull @buf, i64 4)
  // CHECK-OPT-NEXT: [[SWITCHVAL:%.+]] = tail call i64 @llvm.cheri.cap.address.get.i64(ptr addrspace(200) [[CAP]])
  // CHECK-OPT-NEXT: switch i64 [[SWITCHVAL]], label {{%.+}} [
