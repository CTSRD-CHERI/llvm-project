...
// RUN: %riscv64_bakewell_purecap_cc1 %s -o - -emit-llvm -O1 -Weverything -Werror -Wno-declaration-after-statement -verify
// RUN: %riscv64_bakewell_purecap_cc1 -target-feature +zcherilevels %s -o - -emit-llvm -O1 -Weverything -Werror -Wno-declaration-after-statement -verify
...
void test(void *__capability cap, char *__capability cap2, __SIZE_TYPE__ i) {
  ...
  _Static_assert(CHERI_PERM_ELEVATE_LEVEL != 0, "must be defined");
  _Static_assert(CHERI_PERM_STORE_LEVEL != 0, "must be defined");
  _Static_assert(CHERI_PERM_CAPABILITY_LEVEL != 0, "must be defined");
#endif
