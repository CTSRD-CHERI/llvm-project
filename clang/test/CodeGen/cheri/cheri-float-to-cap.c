...
double test_intcap(void) {
  ...
  // CHECK: [[TMP:%.+]] = load double, double addrspace(200)* [[D]], align 8
  // CHECK: [[CONV:%.+]] = fptosi double [[TMP]] to i64
  // CHECK: [[TMP_CAP:%.+]] = call i8 addrspace(200)* @llvm.cheri.cap.address.set.i64(i8 addrspace(200)* null, i64 [[CONV]])
  ...
  // CHECK: [[CAP_OFFSET:%.+]] = call i64 @llvm.cheri.cap.address.get.i64(i8 addrspace(200)* [[TMP_CAP2]])
