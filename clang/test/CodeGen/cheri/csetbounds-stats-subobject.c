...
// PURECAP-CSV-NEXT: 0,4,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-1]]:21","Add subobject bounds","addrof operator on int"
  do_stuff_with_nested(&s->n);
// PURECAP-CSV-NEXT: 0,12,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-1]]:24","Add subobject bounds","addrof operator on struct Nested"
  do_stuff_with_float(&s->f1);
// PURECAP-CSV-NEXT: 0,4,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-1]]:23","Add subobject bounds","addrof operator on float"
}

void test_subobject_addrof_hybrid(struct Nested * __capability cap) {
  do_stuff_with_int_capability(&cap->a);
  // HYBRID-CSV-NEXT: 0,4,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-1]]:32","Add subobject bounds","addrof operator on int"
  // PURECAP-CSV-NEXT: 0,4,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-2]]:32","Add subobject bounds","addrof operator on int"
}
...
// PURECAP-CSV-NEXT: 2,4,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-1]]:23","Add subobject bounds","addrof operator on float"
  do_stuff_with_nested(&GlobalStruct.n);
// PURECAP-CSV-NEXT: 2,12,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-1]]:24","Add subobject bounds","addrof operator on struct Nested"
}
...
void test_subobject_addrof_alignas(struct Nested * __capability cap) {
  ...
  // PURECAP-CSV-NEXT: 7,4,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-1]]:21","Add subobject bounds","addrof operator on int"
  do_stuff_with_int_capability(&onstack128.a);
  // PURECAP-CSV-NEXT: 7,4,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-1]]:32","Add subobject bounds","addrof operator on int"
  do_stuff_with_int(&onstack128.c);
  // PURECAP-CSV-NEXT: 3,4,?,"{{.+}}/csetbounds-stats-subobject.c:[[@LINE-1]]:21","Add subobject bounds","addrof operator on int"
