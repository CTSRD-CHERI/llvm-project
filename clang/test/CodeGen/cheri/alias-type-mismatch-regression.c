...
// RUN: %cheri_purecap_cc1 -mllvm -cheri-cap-table-abi=pcrel -std=gnu99 -o - -O0 -S %s | FileCheck %s -check-prefix ASM
// RUN: %cheri_purecap_cc1 -mllvm -cheri-cap-table-abi=legacy -std=gnu99 -o - -O0 -S %s | FileCheck %s -check-prefix ASM

// RUN: %cheri_purecap_cc1 -mllvm -cheri-cap-table-abi=legacy -std=gnu99 -o - -O0 -emit-obj %s | llvm-objdump -t -d - | FileCheck %s -check-prefix DUMP
void __ledf2() {}
