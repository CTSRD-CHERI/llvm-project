...
void test(struct WithBoundsPls* w, struct NoBoundsPls* n, struct HasMemberOfTypeNoBoundsPls* t, struct HasFieldWithOptOut* f) {
  ...
  // CHECK-NEXT: subscript 'struct NoBoundsPls *' subobj bounds check: array subscript on non-array type -> not setting bounds
...
void test_stringbuf(int next_free) {
  ...
  // CHECK-NEXT: subscript 'char *' subobj bounds check: array subscript on non-array type -> not setting bounds
  ...
  // CHECK-NEXT: subscript 'char * __attribute__((cheri_no_subobject_bounds))' subobj bounds check: array subscript on non-array type -> not setting bounds
...
void test_past_end_macro(struct fake_vla *fv) {
  ...
  // CHECK-NEXT: subscript 'void *[1]' subobj bounds check: got MemberExpr -> subscript on constant size array -> setting bounds for 'void *[1]' subscript to {{16|32}}
  ...
  // CHECK-NEXT: decay 'void *[1]' subobj bounds check: __builtin_no_change_bounds() expression -> not setting bounds
  // CHECK-NEXT: subscript 'typeof (*(fv->data)) *' subobj bounds check: array subscript on non-array type -> not setting bounds
}
