...
void check_sub(__intcap_t cap1, __intcap_t cap2, ptrdiff_t ptrdiff) {
  ...
  // offset-warning@-1{{'__intcap_t' subtraction may yield incorrect results if the values are derived from pointers since it will only consider the capability offset}}
  // offset-note@-2{{in order to perform pointer arithmetic}}
  __intcap_t cap_and_int = cap1 - ptrdiff;
  __intcap_t cap_and_cap = cap1 - cap2;
  // offset-warning@-1{{'__intcap_t' subtraction may yield incorrect results if the values are derived from pointers since it will only consider the capability offset}}
  // address-warning@-2{{'__intcap_t' subtraction may yield incorrect results if the values are derived from pointers since it may result in an unintended tagged value}}
