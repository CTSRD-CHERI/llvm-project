...
// RUN: %cheri_purecap_cc1 -cheri-bounds=subobject-safe -O2 -emit-llvm %s -o - -Rcheri-subobject-bounds -verify -mllvm -stats 2>%t.dbg | FileCheck %s
// RUN: FileCheck --input-file=%t.dbg %s -check-prefix DBG -implicit-check-not cheri-bounds
// DBG: ... Statistics Collected ...
// DBG:  10 cheri-bounds - Number of array-to-pointer-decays checked for tightening bounds
// DBG:  5 cheri-bounds - Number of array-to-pointer-decays where remaining allocation size was used
...
// CHECK-NEXT:    ret void
//
void test_unsized_global_decay(void) {
  overflow_buffer(bhnd_sprom_layouts); // expected-remark{{setting bounds for array decay on 'int[]' to remaining bytes (array decay on variable size type)}}
}
