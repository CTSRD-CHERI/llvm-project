// RUN: %cheri_cc1 -Wno-error=cheri-capability-misuse -ast-dump -o - %s | FileCheck %s -check-prefix AST
// RUN: %cheri_cc1 -Wno-error=cheri-capability-misuse -verify -emit-llvm -o - %s | FileCheck %s --check-prefix=HYBRID
// RUN: %cheri_purecap_cc1 -Wno-error=cheri-capability-misuse -verify=purecap -emit-llvm -o - %s | FileCheck %s --check-prefix=PURECAP
...
char *__capability global_ptr_data = &extern_data;
// HYBRID-NEXT: @global_ptr_data = global i8 addrspace(200)* addrspacecast (i8* @extern_data to i8 addrspace(200)*), align 16
// PURECAP-NEXT: @global_ptr_data = addrspace(200) global i8 addrspace(200)* @extern_data, align 16
// AST-LABEL: VarDecl {{.+}} global_ptr_data 'char * __capability' cinit
...
// FIXME: this is incorrect: purecap-note@-2{{insert cast to intptr_t to silence this warning}}
