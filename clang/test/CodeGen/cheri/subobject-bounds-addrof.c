...
void test_subobject_addrof_basic(struct WithNested* s) {
  ...
  do_stuff_with_int(TAKE_ADDRESS(s->n.a)); // expected-remark {{setting sub-object bounds for pointer to 'int' to 4 bytes}}
  ...
  do_stuff_with_nested(TAKE_ADDRESS(s->n)); // expected-remark {{setting sub-object bounds for pointer to 'struct Nested' to 12 bytes}}
  ...
  do_stuff_with_float(TAKE_ADDRESS(s->f1)); // expected-remark {{setting sub-object bounds for pointer to 'float' to 4 bytes}}
...
void test_atomic(atomic_int_t* array, struct WithAtomicInt *s) {
  ...
  do_stuff_with_void_ptr(&onstack.value); // expected-remark {{setting sub-object bounds for pointer to 'atomic_int_t' (aka '_Atomic(int)') to 4 bytes}}
  ...
  do_stuff_with_void_ptr(&s->value); // expected-remark {{setting sub-object bounds for pointer to 'atomic_int_t' (aka '_Atomic(int)') to 4 bytes}}
  // DBG-NEXT: subobj bounds check: got MemberExpr -> unwrapping _Atomic type -> Found scalar type -> setting bounds for 'atomic_int_t' addrof to 4
