...
// RUN: %clang_cc1 -no-opaque-pointers -triple i386-pc-linux-gnu -emit-llvm -o - %s -verify | FileCheck -check-prefix=CHECKBASIC %s
// RUN: %clang_cc1 -no-opaque-pointers -triple armv7a-eabi -mfloat-abi hard -emit-llvm -o - %s -verify | FileCheck -check-prefix=CHECKCC %s
// RUN: %clang_cc1 -no-opaque-pointers -triple armv7a-eabi -mfloat-abi hard -S -o - %s -verify | FileCheck -check-prefix=CHECKASM %s
// RUN: %clang_cc1 -no-opaque-pointers -triple aarch64-linux-gnu -emit-llvm -o - %s -verify | FileCheck -check-prefix=CHECKGLOBALS %s
