...
// RUN: %clang -E -target cheri-unknown-freebsd11 -cheri -mabi=purecap -fno-pic %s -o /dev/null
// RUN: %clang -E -### -target cheri-unknown-freebsd11 -cheri -mabi=purecap -fno-pic \
// RUN:   %s 2>&1 | FileCheck --check-prefix=PREPROCESS_FNOPIC %s

// Also don't make it an error for cc1as:
// RUN: echo "" > %t.S
// RUN: %clang -### -target cheri-unknown-freebsd11 -cheri -mabi=purecap -fno-pic \
// RUN:  -xassembler-with-cpp -c %t.S 2>&1 | FileCheck --check-prefix=PURECAP_ASM_FNOPIC %s
// RUN: %clang -c -target cheri-unknown-freebsd11 -cheri -mabi=purecap -fno-pic %t.S
