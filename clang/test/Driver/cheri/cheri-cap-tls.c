...
// RUN: %cheri_purecap_clang -cheri-cap-table-abi=legacy %s -c -o - -### 2>&1 | FileCheck -check-prefix LEGACY-ABI %s
// Defaults to cap-equiv when cap table is enabled
// RUN: %cheri_purecap_clang -cheri-cap-table-abi=pcrel %s -c -o - -### 2>&1 | FileCheck -check-prefix PCREL-ABI %s
...
// LEGACY-ABI: "-mllvm" "-cheri-cap-table-abi=legacy"
// LEGACY-ABI-NOT: -cheri-cap-tls-abi=
//  "-mllvm" "-cheri-cap-table-abi=pcrel" implies "-mllvm" "-cheri-cap-tls-abi=cap-equiv" unless overridden
// PCREL-ABI: "-mllvm" "-cheri-cap-table-abi=pcrel"
// PCREL-ABI-NOT: -cheri-cap-tls-abi=
