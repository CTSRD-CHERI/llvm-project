...
struct packed_cap_struct {
  int i;
  struct cap_struct cap;  // expected-warning {{Under aligned structure field in packed structure containing capabilities will trap if capability field is accessed}}
} __attribute__((packed));
...
struct bad_uintptr_array_1 { // expected-note{{Add __attribute__((aligned(32))) to ensure sufficient alignment}}
  char before[sizeof(void*)];
  __UINTPTR_TYPE__ cap; // expected-warning {{Under aligned capability field in packed structure will trap if struct is used in an array}}
  char after[1];
} __attribute__((packed));
...
struct bad_cap_struct_array_1 { // expected-note{{Add __attribute__((aligned(32))) to ensure sufficient alignment}}
  char before[sizeof(void*)];
  struct cap_struct cap; // expected-warning {{Under aligned capability field in packed structure will trap if struct is used in an array}}
  char after[1];
} __attribute__((packed));
