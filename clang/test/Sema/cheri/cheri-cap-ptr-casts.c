...
void a() {
  ...
  // PURECAP-AST: CStyleCastExpr {{.*}} {{.*}} 'int * __capability *' <BitCast>{{$}}
...
void f() {
  ...
  // HYBRID-AST: CStyleCastExpr {{.*}} {{.*}} 'char * __capability':'char * __capability' <PointerToCHERICapability>{{$}}
  // PURECAP-AST: CStyleCastExpr {{.*}} {{.*}} 'char * __capability':'char *' <NoOp>{{$}}
  // AST-NEXT: ImplicitCastExpr {{.*}} {{.*}} 'char *' <ArrayToPointerDecay> part_of_explicit_cast{{$}}
  fp = (__cheri_tocap void (* __capability)()) f;
  // HYBRID-AST: CStyleCastExpr {{.*}} {{.*}} 'void (* __capability)()':'void (* __capability)()' <PointerToCHERICapability>{{$}}
  // PURECAP-AST: CStyleCastExpr {{.*}} {{.*}} 'void (* __capability)()':'void (*)()' <NoOp>{{$}}
...
void fromcap(void *__capability voidcap) {
  ...
  // HYBRID-AST-NEXT: ImplicitCastExpr {{.*}} 'char * __capability':'char * __capability' <LValueToRValue> part_of_explicit_cast{{$}}
  // HYBRID-AST-NEXT: DeclRefExpr {{.*}} 'char * __capability':'char * __capability' lvalue Var {{.+}} 'x' 'char * __capability':'char * __capability'{{$}}
  // PURECAP-AST: CStyleCastExpr {{.*}} {{.*}} 'char *' <NoOp>{{$}}
  // PURECAP-AST-NEXT: ImplicitCastExpr {{.*}} 'char * __capability':'char *' <LValueToRValue> part_of_explicit_cast{{$}}
  // PURECAP-AST-NEXT: DeclRefExpr {{.*}} 'char * __capability':'char *' lvalue Var {{.+}} 'x' 'char * __capability':'char *'{{$}}
  ...
  // HYBRID-AST: CStyleCastExpr {{.*}} {{.*}} 'void *' <CHERICapabilityToPointer>{{$}}
  // HYBRID-AST-NEXT: ImplicitCastExpr {{.*}} 'void * __capability':'void * __capability' <LValueToRValue> part_of_explicit_cast{{$}}
  // HYBRID-AST-NEXT: DeclRefExpr {{.*}} 'void * __capability':'void * __capability' lvalue ParmVar {{.+}} 'voidcap' 'void * __capability':'void * __capability'{{$}}
  // PURECAP-AST: CStyleCastExpr {{.*}} {{.*}} 'void *' <NoOp>{{$}}
  // PURECAP-AST-NEXT: ImplicitCastExpr {{.*}} 'void * __capability':'void *' <LValueToRValue> part_of_explicit_cast{{$}}
  // PURECAP-AST-NEXT: DeclRefExpr {{.*}} 'void * __capability':'void *' lvalue ParmVar {{.+}} 'voidcap' 'void * __capability':'void *'{{$}}
