...
struct QMapNodeBase
{
    ...
    bool comparisonOp() const { return (p & Mask) == Black; } // expected-warning {{binary expression on capability and non-capability types}}
    bool comparisonOp2() const { return Red == (p & Mask); } // expected-warning {{binary expression on capability and non-capability types}}

    bool capRHS() const { return (Black | p) == p; } // expected-warning {{binary expression on capability and non-capability types}}
};
