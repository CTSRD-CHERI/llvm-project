...
config.substitutions.append( ('%cheri_purecap_cc1', '%cheri_cc1 -triple cheri-unknown-freebsd -target-abi purecap') )

if config.cheri_is_128:
    config.available_features.add("cheri_is_128")
    clang_cc1_cmd += ' -mllvm -cheri128'
    cheri256_cc1_substitution += ' -mllvm -cheri256 -mllvm -cheri-test-mode'
    config.substitutions.append( ('%cheri_cc1', cheri128_cc1_substitution) )
else:
    config.available_features.add("cheri_is_256")
    cheri128_cc1_substitution += ' -mllvm -cheri-test-mode'
    config.substitutions.append( ('%cheri_cc1', cheri256_cc1_substitution) )
