...
namespace testCanonicalTemplate {
  ...
  // CHECK-NEXT: `-VarTemplateSpecializationDecl 0x{{.+}} <line:[[@LINE-18]]:28, col:43> col:43 referenced TestVarTemplate 'const int' implicit_instantiation static
  // CHECK-NEXT:   `-TemplateArgument type 'int'

  // CHECK:     VarTemplateSpecializationDecl 0x{{.+}} <{{.+}}:[[@LINE-21]]:28, col:43> col:43 referenced TestVarTemplate 'const int' implicit_instantiation static
  // CHECK-NEXT:`-TemplateArgument type 'int'
  // CHECK-NEXT:  `-BuiltinType 0x{{.+}} 'int'
  ...
  // CHECK:      VarTemplateSpecializationDecl 0x{{.+}} parent 0x{{.+}} prev 0x{{.+}} <{{.+}}:[[@LINE-29]]:3, col:34> col:14 referenced TestVarTemplate 'const int' implicit_instantiation cinit
  // CHECK-NEXT: |-TemplateArgument type 'int'
  // CHECK-NEXT: | `-BuiltinType 0x{{.+}} 'int'
  // CHECK-NEXT: `-InitListExpr 0x{{.+}} <col:32, col:34> 'int'
} 
