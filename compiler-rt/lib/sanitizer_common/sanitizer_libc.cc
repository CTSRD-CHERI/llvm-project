...
usize internal_strlcat(char *dst, const char *src, usize maxlen) {
  const usize srclen = internal_strlen(src);
  const usize dstlen = internal_strnlen(dst, maxlen);
...
char *internal_strstr(const char *haystack, const char *needle) {
  ...
  usize len1 = internal_strlen(haystack);
  usize len2 = internal_strlen(needle);
  if (len1 < len2) return nullptr;
  for (usize pos = 0; pos <= len1 - len2; pos++) {
