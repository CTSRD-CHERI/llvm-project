 ...
 public:
  ...
  class RuntimeSelectAllocatorCache {
   ...
   public:
    ...
    void *Allocate(RuntimeSelectAllocator *allocator, usize class_id) {
      if (allocator->use_first_allocator)
        return a1.Allocate(&allocator->a1, class_id);
      return a2.Allocate(&allocator->a2, class_id);
    }

    void Deallocate(RuntimeSelectAllocator *allocator, usize class_id, void *p) {
      if (allocator->use_first_allocator)
  ...
  void Init(s32 release_to_os_interval_ms) {
    ...
    use_first_allocator = GetMaxVirtualAddress() < (((vaddr)1ULL << 48) - 1);
