...
template <typename A, typename R>
typename A::pc_t UnwindCursor<A, R>::getIP() {
...
#if defined(_LIBUNWIND_SUPPORT_DWARF_UNWIND)
template <typename A, typename R>
bool UnwindCursor<A, R>::getInfoFromDwarfSection(pc_t pc,
                                                const UnwindInfoSections &sects,
  ...
  if (foundFDE) {
    ...
    if (CFI_Parser<A>::parseFDEInstructions(_addressSpace, fdeInfo, cieInfo, pc.address(),
                                            R::getArch(), &prolog)) {
      // Save off parsed FDE info
      _info.start_ip          = fdeInfo.pcStart.mutableValue(); // FIXME: could be immutable
  ...
  #endif
        DwarfFDECache<A>::add(sects.dso_base, fdeInfo.pcStart.address(), fdeInfo.pcEnd.address(),
                              fdeInfo.fdeStart);
      }
      return true;
    }
...
#if defined(_LIBUNWIND_SUPPORT_COMPACT_UNWIND)
template <typename A, typename R>
bool UnwindCursor<A, R>::getInfoFromCompactEncodingSection(pc_t pc_raw,
                                              const UnwindInfoSections &sects) {
...
void UnwindCursor<A, R>::setInfoBasedOnIPRegister(bool isReturnAddress) {
  ...
  if (cachedFDE != 0) {
    ...
    const char *msg = CFI_Parser<A>::decodeFDE(_addressSpace, pc, cachedFDE, &fdeInfo, &cieInfo);
    if (msg == NULL) {
      typename CFI_Parser<A>::PrologInfo prolog;
      if (CFI_Parser<A>::parseFDEInstructions(_addressSpace, fdeInfo, cieInfo,
                                              pc.address(), R::getArch(), &prolog)) {
        // save off parsed FDE info
        _info.start_ip         = fdeInfo.pcStart.mutableValue(); // FIXME: could be immutable
        _info.end_ip           = fdeInfo.pcEnd.mutableValue(); // FIXME: could be immutable
        _info.lsda             = fdeInfo.lsda;
