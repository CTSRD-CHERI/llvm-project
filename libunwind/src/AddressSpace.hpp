...
public:
   ...
   * since they might be sealed entry capabilities that cannot be modified
   */
  class LocalProgramCounter {
    void* value;
#ifdef __CHERI_PURE_CAPABILITY__
    // the actual pc might be a sentry but libunwind needs to modify it sometimes
    uint64_t addend = 0;
...
#ifdef __CHERI_PURE_CAPABILITY__
      return __builtin_cheri_type_get((void*)value) != UINT64_MAX; // -1 is unsealed
#else
      return false;
#endif
    }
...
inline bool operator op(LocalAddressSpace::pint_t lhs, const LocalAddressSpace::pc_t &rhs) { \
  return (__cheri_addr LocalAddressSpace::addr_t)lhs op rhs.address(); \
} \
inline bool operator op(const LocalAddressSpace::pc_t& lhs, LocalAddressSpace::pint_t rhs) { \
  return lhs.address() op (__cheri_addr LocalAddressSpace::addr_t)rhs; \
} \
inline bool operator op(LocalAddressSpace::addr_t lhs, const LocalAddressSpace::pc_t &rhs) { \
  return lhs op rhs.address(); \
} \
inline bool operator op(const LocalAddressSpace::pc_t& lhs, LocalAddressSpace::addr_t rhs) { \
  return lhs.address() op rhs; \
} \
inline bool operator op(const LocalAddressSpace::pc_t& lhs, const LocalAddressSpace::pc_t &rhs) { \
  ...
  int found = dl_iterate_phdr(
      [](struct dl_phdr_info *pinfo, size_t, void *data) -> int {
        ...
        if (cbdata->targetAddr < pinfo->dlpi_addr) {
          CHERI_DBG("0x%jx out of bounds of %#p (%s)\n", (uintmax_t)cbdata->targetAddr.address(), (void*)pinfo->dlpi_addr, pinfo->dlpi_name);
          return false;
        }
#ifdef __CHERI_PURE_CAPABILITY__
        ...
        if (__builtin_cheri_length_get((void *)pinfo->dlpi_addr) +
            ...
            cbdata->targetAddr) {
          CHERI_DBG("%#p out of bounds of %#p (%s)\n", (void*)cbdata->targetAddr.get(), (void*)pinfo->dlpi_addr, pinfo->dlpi_name);
          return false;
        }
#endif
...
#if _LIBUNWIND_USE_DLADDR
  Dl_info dyldInfo;
  CHERI_DBG("%s(0x%jx: %#p))\n", __func__, (uintmax_t)ip.address(), (void*)ip.get());
