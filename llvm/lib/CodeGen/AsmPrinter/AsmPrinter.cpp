...
void AsmPrinter::emitGlobalVariable(const GlobalVariable *GV) {
  ...
  if (GVKind.isBSS() && MAI->hasMachoZeroFillDirective() &&
    ...
    OutStreamer->emitZerofill(TheSection, GVSym, Size, Alignment.value(), TailPadding);
    return;
  }
  ...
  if (GVKind.isBSSLocal() &&
    ...
    if (MAI->getLCOMMDirectiveAlignmentType() != LCOMM::NoAlignment) {
      // .lcomm _foo, 42
      OutStreamer->emitLocalCommonSymbol(GVSym, Size, Alignment.value(), TailPadding);
      return;
    }
    ...
    for (const MDOperand &MDO : MD.operands()) {
      if (auto *S = dyn_cast<MDString>(MDO)) {
        ...
        for (const MDOperand &AuxMDO : AuxMDs->operands()) {
          ...
          emitGlobalConstant(F.getParent()->getDataLayout(), C->getValue(), 0);
        }
      }
    }
  };
