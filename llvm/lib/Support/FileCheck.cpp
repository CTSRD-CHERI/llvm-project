...
static uint64_t div(uint64_t LeftOp, uint64_t RightOp) {
  return LeftOp / RightOp;
}
...
FileCheckPattern::parseBinop(StringRef &Expr, const SourceMgr &SM) const {
  ...
  if (Expr.consumeInteger(10, RightOp)) {
    if (Expr.consume_front("#")) {
      Expected<FileCheckNumericVariable *> RightParseResult =
          parseNumericVariableUse(Expr, SM);
      if (!RightParseResult)
        return RightParseResult.takeError();
      RightOp = *(*RightParseResult)->getValue();
    } else if (Expr.consume_front("(#") && Expr.consume_back(")")) {
      Optional<FileCheckNumericVariable *> NewVar;
      Expected<FileCheckExpression *> SubExprParseResult =
          parseNumericSubstitutionBlock(Expr, NewVar, SM);
      if (NewVar)
        return FileCheckErrorDiagnostic::get(
            SM, Expr, "invalid definition in nested expression '" + Expr + "'");
      if (!SubExprParseResult)
        return SubExprParseResult.takeError();
      RightOp = *(*SubExprParseResult)->eval();
    } else {
      return FileCheckErrorDiagnostic::get(
