...
FileCheckPattern::parseBinop(StringRef &Expr,
  ...
  std::unique_ptr<FileCheckExpressionAST> RightOp = nullptr;
  if (!IsLegacyLineExpr && Expr.consume_front("(")) {
    ...
    Expected<std::unique_ptr<FileCheckExpressionAST>> SubExprResult =
      parseNumericSubstitutionBlock(SubExpr, DefinedNumericVariable, false, SM);
    if (!SubExprResult)
    ...
    if (DefinedNumericVariable) {
      ...
      return FileCheckErrorDiagnostic::get(
          SM, SubExprLoc,
          "invalid definition of numeric variable '" + Name +
    ...
    if (I == E)
      return FileCheckErrorDiagnostic::get(
          SM, Expr, "missing ')' at end of nested expression");
