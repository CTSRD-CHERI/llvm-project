...
SDValue RISCVTargetLowering::getDynamicTLSAddr(GlobalAddressSDNode *N,
  ...
  SDValue Load;
  if (RISCVABI::isCheriPureCapABI(Subtarget.getTargetABI()))
    Load = DAG.getNode(RISCVISD::CLC_TLS_GD, DL, Ty, Addr);
  else
    Load =
...
SDValue RISCVTargetLowering::lowerGlobalTLSAddress(SDValue Op,
  ...
  case TLSModel::GeneralDynamic:
    Addr = (DAG.getTarget().useTLSDESC() &&
            !RISCVABI::isCheriPureCapABI(Subtarget.getTargetABI()))
               ? getTLSDescAddr(N, DAG)
  ...
  case ISD::MGATHER: {
    ...
    if (Index.getOpcode() == ISD::BUILD_VECTOR &&
      ...
      if (std::optional<VIDSequence> SimpleVID = isSimpleVIDSequence(Index);
        ...
        SDValue BasePtr = DAG.getMemBasePlusOffset(
            MGN->getBasePtr(), TypeSize::getFixed(Addend), DL);
...
SDValue RISCVTargetLowering::LowerFormalArguments(
  ...
  for (unsigned i = 0, e = ArgLocs.size(), InsIdx = 0; i != e; ++i, ++InsIdx) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      ArgValue =
          unpackF64OnRV32DSoftABI(DAG, Chain, VA, ArgLocs[++i], DL, PtrVT);
    } else if (VA.isRegLoc())
