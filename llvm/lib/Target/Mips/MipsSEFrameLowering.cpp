...
int MipsSEFrameLowering::createUnsafeEndObject(MachineFunction &MF) const {
  ...
  unsigned CapSize =
      static_cast<const MipsSubtarget &>(MF.getSubtarget()).getCapSizeInBytes();

  // This is the 'prev' pointer for the linked list of stacks
  return MFI.CreateStackObject(
      ABI.GetTABILayout()->GetStackPointerSize_Prev() * CapSize,
...
void MipsSEFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (ABI.IsCheriOS()) {
    ...
    CapSize = static_cast<const MipsSubtarget &>(MF.getSubtarget())
                  .getCapSizeInBytes();
...
void MipsSEFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (MFI.hasStaticUnsafeObjects() && ABI.IsCheriOS()) {
    ...
    unsigned CapSize = static_cast<const MipsSubtarget &>(MF.getSubtarget())
                           .getCapSizeInBytes();
