...
namespace rdf {
  ...
  struct NodeAllocator {
    // Amount of storage for a single node; must be a power of two.
    enum { NodeMemSize = sizeof(void *) <= 8 ? 32 : 64 };
  ...
  // The allocator allocates chunks of 32/64 bytes for each node. The fact that
  // each node takes 32/64 bytes in memory is used for fast translation between
