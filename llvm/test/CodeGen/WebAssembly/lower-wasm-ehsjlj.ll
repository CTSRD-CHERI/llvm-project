...
; RUN: llc < %s -wasm-enable-eh -wasm-enable-sjlj -exception-model=wasm -mattr=+exception-handling -verify-machineinstrs -o /dev/null
...
; CHECK-NEXT:   %5 = call i8* @__cxa_begin_catch(i8* %exn) #[[#]] [ "funclet"(token %2) ]
