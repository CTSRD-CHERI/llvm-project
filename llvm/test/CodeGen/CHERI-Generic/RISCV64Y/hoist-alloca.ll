...
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d -o %t.mir -stop-before=early-machinelicm < %s
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d -run-pass=early-machinelicm -debug-only=machinelicm %t.mir -o /dev/null 2>%t.dbg
...
; MACHINELICM-DBG: Hoisting [[INC:%[0-9]+]]:gpcr = ADDIY %stack.0.buf1, 0
; MACHINELICM-DBG-NEXT:  from %bb.2 to %bb.0
; MACHINELICM-DBG: Hoisting [[BOUNDS:%[0-9]+]]:gpcr = YBNDSRW [[INC]]:gpcr, [[IMM]]:gpr
; MACHINELICM-DBG-NEXT:  from %bb.2 to %bb.0
; MACHINELICM-DBG: Hoisting [[IMM:%[0-9]+]]:gpr = ADDI $x0, 88
; MACHINELICM-DBG-NEXT:  from %bb.2 to %bb.0
; MACHINELICM-DBG: Hoisting [[INC:%[0-9]+]]:gpcr = ADDIY %stack.1.buf2, 0
; MACHINELICM-DBG-NEXT:  from %bb.2 to %bb.0
; MACHINELICM-DBG: Hoisting [[BOUNDS:%[0-9]+]]:gpcr = YBNDSRW [[INC]]:gpcr, [[IMM]]:gpr
; MACHINELICM-DBG-NEXT:  from %bb.2 to %bb.0
...
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d -O1 -o - < %s | FileCheck %s
...
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    addiy csp, csp, -656
; CHECK-NEXT:    sy cra, 640(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs0, 624(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs1, 608(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs2, 592(csp) # 16-byte Folded Spill
; CHECK-NEXT:    li s2, 100
; CHECK-NEXT:    li a0, 492
; CHECK-NEXT:    addiy ca1, csp, 100
; CHECK-NEXT:    ybndsrw cs0, ca1, a0
; CHECK-NEXT:    li a0, 88
; CHECK-NEXT:    addiy ca1, csp, 12
; CHECK-NEXT:    ybndsrw cs1, ca1, a0
; CHECK-NEXT:  .LBB0_1: # %for.body
; CHECK-NEXT:    # =>This Inner Loop Header: Depth=1
; CHECK-NEXT:    ymv ca0, cs0
; CHECK-NEXT:    ymv ca1, cs1
; CHECK-NEXT:    call call
; CHECK-NEXT:    addiw s2, s2, -1
; CHECK-NEXT:    bnez s2, .LBB0_1
; CHECK-NEXT:  # %bb.2: # %for.cond.cleanup
; CHECK-NEXT:    ly cra, 640(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs0, 624(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs1, 608(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs2, 592(csp) # 16-byte Folded Reload
; CHECK-NEXT:    addiy csp, csp, 656
; CHECK-NEXT:    ret
entry:
...
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    addiy csp, csp, -672
; CHECK-NEXT:    sy cra, 656(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs0, 640(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs1, 624(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs2, 608(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs3, 592(csp) # 16-byte Folded Spill
; CHECK-NEXT:    mv s0, a0
...
; CHECK-NEXT:    ly cra, 656(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs0, 640(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs1, 624(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs2, 608(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs3, 592(csp) # 16-byte Folded Reload
; CHECK-NEXT:    addiy csp, csp, 672
; CHECK-NEXT:    ret
entry:
  %buf1 = alloca [123 x i32], align 4, addrspace(200)
