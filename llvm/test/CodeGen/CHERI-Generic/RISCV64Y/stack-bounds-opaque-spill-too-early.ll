...
; REQUIRES: asserts
; RUN: opt -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d -cheri-bound-allocas -o - -S %s -debug-only=cheri-bound-allocas 2>%t.dbg| FileCheck %s
; RUN: FileCheck %s -input-file=%t.dbg -check-prefix DBG
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d %s -o - | FileCheck %s -check-prefix ASM
...
; ASM-NEXT:    call cheribsdtest_dynamic_identity_cap
; ASM-NEXT:    ly ca1, 0(csp)
; ASM-NEXT:    ymv ca2, ca0
; ASM-NEXT:    ymv ca0, ca1
; ASM-NEXT:    ymv ca1, ca2
; ASM-NEXT:    call cheribsdtest_check_cap_eq
