...
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d -O2 --cheri-stack-bounds-single-intrinsic-threshold=0 < %s | %cheri_FileCheck %s --check-prefixes=CHECK
; Always use a single intrinsic for the calls (should result in same codegen)
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d -O2 --cheri-stack-bounds-single-intrinsic-threshold=0 < %s | %cheri_FileCheck %s --check-prefixes=CHECK
; RUN: sed 's/addrspace(200)/addrspace(0)/g' %s | llc -mtriple=riscv64 --relocation-model=pic -target-abi lp64d -mattr=+y,+zyhybrid,+f,+d | FileCheck --check-prefix HYBRID %s
...
; CHECK:       # %bb.0:
; CHECK-NEXT:    addiy csp, csp, -48
; CHECK-NEXT:    sy cra, 32(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs0, 16(csp) # 16-byte Folded Spill
; CHECK-NEXT:    addiy ca0, csp, 12
; CHECK-NEXT:    ybndsiw cs0, ca0, 4
; CHECK-NEXT:    li a0, 123
; CHECK-NEXT:    sw a0, 12(csp)
; CHECK-NEXT:    call foo
; CHECK-NEXT:    ymv ca0, cs0
; CHECK-NEXT:    call one_arg
; CHECK-NEXT:    ly cra, 32(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs0, 16(csp) # 16-byte Folded Reload
; CHECK-NEXT:    addiy csp, csp, 48
; CHECK-NEXT:    ret
;
...
; CHECK:       # %bb.0:
; CHECK-NEXT:    addiy csp, csp, -64
; CHECK-NEXT:    sy cra, 48(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs0, 32(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs1, 16(csp) # 16-byte Folded Spill
; CHECK-NEXT:    addiy ca0, csp, 12
; CHECK-NEXT:    ybndsiw cs0, ca0, 4
; CHECK-NEXT:    addiy ca0, csp, 8
; CHECK-NEXT:    ybndsiw cs1, ca0, 4
; CHECK-NEXT:    call foo
; CHECK-NEXT:    ymv ca0, cs0
; CHECK-NEXT:    call one_arg
; CHECK-NEXT:    ymv ca0, cs1
; CHECK-NEXT:    call one_arg
; CHECK-NEXT:    ly cra, 48(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs0, 32(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs1, 16(csp) # 16-byte Folded Reload
; CHECK-NEXT:    addiy csp, csp, 64
; CHECK-NEXT:    ret
;
...
; CHECK-NEXT:    ybndsiw cs1, ca0, 4
; CHECK-NEXT:    call foo
; CHECK-NEXT:    addiy ca1, cs1, 4
; CHECK-NEXT:    addiy ca2, cs1, 1
; CHECK-NEXT:    ymv ca0, cs1
