...
; RUN: opt -data-layout="e-m:e-pf200:128:128:128:64-p:64:64-i64:64-i128:128-n64-S128-A200-P200-G200" -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d "-passes=default<O3>" -S < %s | FileCheck %s --check-prefix=HOIST-OPT
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d -O3 < %s | FileCheck %s
...
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    addiy csp, csp, -96
; CHECK-NEXT:    sy cra, 80(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs0, 64(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs1, 48(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs2, 32(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs3, 16(csp) # 16-byte Folded Spill
; CHECK-NEXT:    sy cs4, 0(csp) # 16-byte Folded Spill
; CHECK-NEXT:    ymv cs0, ca1
; CHECK-NEXT:    addiy ca0, ca1, 4
; CHECK-NEXT:    li s3, -1
; CHECK-NEXT:    li s4, 99
; CHECK-NEXT:    li a1, 4
; CHECK-NEXT:    ybndsrw cs1, cs0, a1
; CHECK-NEXT:    ybndsrw cs2, ca0, a1
; CHECK-NEXT:    j .LBB0_2
...
; CHECK-NEXT:    # in Loop: Header=BB0_2 Depth=1
; CHECK-NEXT:    ymv ca0, cs1
; CHECK-NEXT:    ymv ca1, cs2
; CHECK-NEXT:    call call
; CHECK-NEXT:    j .LBB0_1
; CHECK-NEXT:  .LBB0_4: # %for.cond.cleanup
; CHECK-NEXT:    ly cra, 80(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs0, 64(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs1, 48(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs2, 32(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs3, 16(csp) # 16-byte Folded Reload
; CHECK-NEXT:    ly cs4, 0(csp) # 16-byte Folded Reload
; CHECK-NEXT:    addiy csp, csp, 96
; CHECK-NEXT:    ret
