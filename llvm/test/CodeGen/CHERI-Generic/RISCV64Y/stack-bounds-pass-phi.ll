...
; REQUIRES: asserts
; RUN: opt -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d -cheri-bound-allocas %s -o - -S -cheri-stack-bounds=if-needed \
; RUN:    -cheri-stack-bounds-single-intrinsic-threshold=10 -debug-only=cheri-bound-allocas 2>%t.dbg | FileCheck %s
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d -cheri-stack-bounds=if-needed -O2 -cheri-stack-bounds-single-intrinsic-threshold=10 < %s | %cheri_FileCheck %s -check-prefix ASM
...
; ASM-LABEL: test_phi:
; ASM:       # %bb.0: # %entry
; ASM-NEXT:    addiy csp, csp, -48
; ASM-NEXT:    sy cra, 32(csp) # 16-byte Folded Spill
; ASM-NEXT:    sy cs0, 16(csp) # 16-byte Folded Spill
; ASM-NEXT:    andi a0, a0, 1
; ASM-NEXT:    beqz a0, .LBB0_2
; ASM-NEXT:  # %bb.1: # %block1
; ASM-NEXT:    ymv ca0, cnull
; ASM-NEXT:    li a1, 1
...
; ASM-NEXT:    addiy ca0, csp, 12
; ASM-NEXT:    ybndsiw ca0, ca0, 4
; ASM-NEXT:    addiy ca1, csp, 4
; ASM-NEXT:  .LBB0_3: # %phi_block
; ASM-NEXT:    ybndsiw cs0, ca1, 4
; ASM-NEXT:    call foo
; ASM-NEXT:    ymv ca0, cs0
; ASM-NEXT:    call foo
; ASM-NEXT:    ly cra, 32(csp) # 16-byte Folded Reload
; ASM-NEXT:    ly cs0, 16(csp) # 16-byte Folded Reload
; ASM-NEXT:    addiy csp, csp, 48
; ASM-NEXT:    ret
...
; ASM-NEXT:    sy cra, 16(csp) # 16-byte Folded Spill
; ASM-NEXT:    andi a0, a0, 1
...
; ASM-NEXT:    li a0, 5
; ASM-NEXT:    sw a0, 8(csp)
; ASM-NEXT:    addiy ca0, csp, 8
; ASM-NEXT:  .LBB1_3: # %phi_block
; ASM-NEXT:    ybndsiw ca0, ca0, 4
; ASM-NEXT:    call foo
; ASM-NEXT:    ly cra, 16(csp) # 16-byte Folded Reload
; ASM-NEXT:    addiy csp, csp, 32
; ASM-NEXT:    ret
