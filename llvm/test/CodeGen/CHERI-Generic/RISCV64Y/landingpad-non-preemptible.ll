...
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d --relocation-model=pic < %s -o - | FileCheck %s
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d --relocation-model=pic < %s -o - -filetype=obj | llvm-readobj --relocs --symbols - | FileCheck %s --check-prefix=RELOCS
...
; CHECK-NEXT:    sy cs1, 0(csp) # 16-byte Folded Spill
...
; CHECK-NEXT:    ly cs1, 0(csp) # 16-byte Folded Reload
; CHECK-NEXT:    addiy csp, csp, 48
; CHECK-NEXT:    ret
