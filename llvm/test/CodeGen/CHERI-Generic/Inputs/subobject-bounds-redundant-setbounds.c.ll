...
; RUN: opt -opaque-pointers=0 @PURECAP_HARDFLOAT_ARGS@ -cheri-bound-allocas -debug-only=cheri-bound-allocas -S -o - %s 2>%t.dbg-opt | FileCheck %s
; RUN: FileCheck %s -input-file=%t.dbg-opt -check-prefix DBG
; RUN: llc -opaque-pointers=0 @PURECAP_HARDFLOAT_ARGS@ -debug-only=cheri-bound-allocas -o - %s 2>%t.dbg-llc | FileCheck %s -check-prefix ASM
