...
; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 @PURECAP_HARDFLOAT_ARGS@ -cheri-bound-allocas %s -o - -S -cheri-stack-bounds=if-needed \
; RUN:    -cheri-stack-bounds-single-intrinsic-threshold=10 -debug-only=cheri-bound-allocas 2>%t.dbg | FileCheck %s
; RUN: llc -opaque-pointers=0 @PURECAP_HARDFLOAT_ARGS@ -cheri-stack-bounds=if-needed -O2 -cheri-stack-bounds-single-intrinsic-threshold=10 < %s | %cheri_FileCheck %s -check-prefix ASM
