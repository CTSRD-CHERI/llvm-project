...
; RUN: opt -opaque-pointers=0 @PURECAP_HARDFLOAT_ARGS@ -S -aa-pipeline=basic-aa -passes=gvn -o - %s | FileCheck %s
; RUN: opt -opaque-pointers=0 @PURECAP_HARDFLOAT_ARGS@ -S -aa-pipeline=basic-aa -passes=gvn -o - %s | llc @PURECAP_HARDFLOAT_ARGS@ -O0 -o - | FileCheck %s --check-prefix=ASM
