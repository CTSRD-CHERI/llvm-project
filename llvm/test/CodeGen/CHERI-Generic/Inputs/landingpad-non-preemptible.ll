...
; RUN: llc @PURECAP_HARDFLOAT_ARGS@ --relocation-model=pic < %s -o - -cheri-codeptr-relocs | FileCheck %s --check-prefixes=CHECK,CODEPTR
...
; NOCODEPTR-NEXT:  .chericap  .L_Z8do_catchv$local+(.Ltmp2-.Lfunc_begin0)   #     jumps to .Ltmp2
; CODEPTR-NEXT:  .chericap  .L_Z8do_catchv$local@code+(.Ltmp2-.Lfunc_begin0)   #     jumps to .Ltmp2
