...
; RUN: opt -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+y,+cap-mode,+f -cheri-bound-allocas -debug-only=cheri-bound-allocas -S -o - %s 2>%t.dbg-opt | FileCheck %s
; RUN: FileCheck %s -input-file=%t.dbg-opt -check-prefix DBG
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+y,+cap-mode,+f -debug-only=cheri-bound-allocas -o - %s 2>%t.dbg-llc | FileCheck %s -check-prefix ASM
...
; ASM:       # %bb.0:
; ASM-NEXT:    addiy csp, csp, -64
; ASM-NEXT:    sy cra, 56(csp) # 8-byte Folded Spill
; ASM-NEXT:    sy cs0, 48(csp) # 8-byte Folded Spill
; ASM-NEXT:    li a0, 40
; ASM-NEXT:    addiy ca1, csp, 8
; ASM-NEXT:    ybndsrw cs0, ca1, a0
; ASM-NEXT:    ymv ca0, cs0
; ASM-NEXT:    call use
; ASM-NEXT:    lw a0, 20(cs0)
; ASM-NEXT:    ly cra, 56(csp) # 8-byte Folded Reload
; ASM-NEXT:    ly cs0, 48(csp) # 8-byte Folded Reload
; ASM-NEXT:    addiy csp, csp, 64
; ASM-NEXT:    ret
...
; ASM-LABEL: stack_int:
; ASM:       # %bb.0:
; ASM-NEXT:    addiy csp, csp, -16
; ASM-NEXT:    sy cra, 8(csp) # 8-byte Folded Spill
; ASM-NEXT:    li a0, 1
...
; ASM-NEXT:    ly cra, 8(csp) # 8-byte Folded Reload
; ASM-NEXT:    addiy csp, csp, 16
; ASM-NEXT:    ret
