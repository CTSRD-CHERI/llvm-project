...
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi ilp32f -mattr=+y,+zyhybrid,+f %s -o - | \
; RUN:   FileCheck %s --check-prefix=ASM -DPTR_DIRECTIVE=.word
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi ilp32f -mattr=+y,+zyhybrid,+f %s -filetype=obj -o - | llvm-objdump -r -t - | \
