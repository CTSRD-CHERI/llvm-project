...
; RUN: opt -opaque-pointers=0 -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+xcheri,+cap-mode,+f -cheri-bound-allocas -o - -S %s | FileCheck %s
; RUN: llc -opaque-pointers=0 -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+xcheri,+cap-mode,+f -O0 %s -o - | FileCheck %s -check-prefix ASM
; RUN: llc -opaque-pointers=0 -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+xcheri,+cap-mode,+f -O2 %s -o - | FileCheck %s -check-prefix ASM-OPT
