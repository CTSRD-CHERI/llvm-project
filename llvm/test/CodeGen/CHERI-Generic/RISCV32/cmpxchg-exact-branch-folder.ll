...
; RUN: cat %s | llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+xcheri,+cap-mode,+f -mattr=+a | FileCheck %s
; REQUIRES: asserts
...
; CHECK-NEXT:    clr.w a1, (ca0)
; CHECK-NEXT:    bne a1, a3, .LBB0_3
...
; CHECK-NEXT:    cret
  ...
  ; MIR-NEXT:   renamable $x10 = ANDI killed renamable $x10, 1
  ; MIR-NEXT:   renamable $c10 = CLC_64 killed renamable $c11, 0 :: (load (s64) from %ir.ptr, align 16, addrspace 200)
  ; MIR-NEXT:   dead early-clobber renamable $c11, dead early-clobber renamable $x14 = PseudoCheriCmpXchgCapAddr killed renamable $c10, killed renamable $c13, killed renamable $c12, 5 :: (load store release monotonic (s64) on %ir.ld2, addrspace 200), (load store release monotonic (s64) on %ir.ld1, addrspace 200)
  ; MIR-NEXT:   $x10 = COPY $x0
  ; MIR-NEXT:   PseudoCRET implicit $x10
entry:
  br i1 %cond1, label %if.then, label %if.else
...
; CHECK-NEXT:    cret
  ...
  ; MIR-NEXT:   renamable $x10 = ANDI killed renamable $x10, 1
  ; MIR-NEXT:   renamable $c10 = CLC_64 killed renamable $c11, 0 :: (load (s64) from %ir.ptr, align 16, addrspace 200)
  ; MIR-NEXT:   dead early-clobber renamable $c11, dead early-clobber renamable $x14 = PseudoCheriCmpXchgCapExact killed renamable $c10, killed renamable $c13, killed renamable $c12, 5 :: (load store release monotonic exact (s64) on %ir.ld2, addrspace 200), (load store release monotonic exact (s64) on %ir.ld1, addrspace 200)
  ; MIR-NEXT:   $x10 = COPY $x0
  ; MIR-NEXT:   PseudoCRET implicit $x10
entry:
  br i1 %cond1, label %if.then, label %if.else
...
; CHECK-NEXT:    cret
  ...
  ; MIR-NEXT: {{  $}}
  ; MIR-NEXT:   renamable $c10 = CLC_64 killed renamable $c11, 0 :: (load (s64) from %ir.ptr, align 16, addrspace 200)
  ; MIR-NEXT:   dead early-clobber renamable $c11, dead early-clobber renamable $x14 = PseudoCheriCmpXchgCapExact killed renamable $c10, killed renamable $c13, killed renamable $c12, 5 :: (load store release monotonic exact (s64) on %ir.ld1, addrspace 200)
  ...
  ; MIR-NEXT: {{  $}}
  ; MIR-NEXT:   renamable $c10 = CLC_64 killed renamable $c11, 0 :: (load (s64) from %ir.ptr, align 16, addrspace 200)
  ; MIR-NEXT:   dead early-clobber renamable $c11, dead early-clobber renamable $x14 = PseudoCheriCmpXchgCapAddr killed renamable $c10, killed renamable $c13, killed renamable $c12, 5 :: (load store release monotonic (s64) on %ir.ld2, addrspace 200)
  ; MIR-NEXT: {{  $}}
  ; MIR-NEXT: bb.3.end:
  ; MIR-NEXT:   $x10 = COPY $x0
  ; MIR-NEXT:   PseudoCRET implicit $x10
