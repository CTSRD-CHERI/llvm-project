...
; RUN: opt -opaque-pointers=0 -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+cap-mode,+f,+d -cheri-bound-allocas -o - -S %s | FileCheck %s
; RUN: llc -opaque-pointers=0 -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+cap-mode,+f,+d -O0 %s -o - | FileCheck %s -check-prefix ASM
; RUN: llc -opaque-pointers=0 -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+cap-mode,+f,+d -O2 %s -o - | FileCheck %s -check-prefix ASM-OPT
