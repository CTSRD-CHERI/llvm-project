...
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi lp64d -mattr=+zcheripurecap,+zcherihybrid,+f,+d %s -o - | \
; RUN: FileCheck %s --check-prefix=ASM -DPTR_DIRECTIVE=.quad
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi lp64d -mattr=+zcheripurecap,+zcherihybrid,+f,+d %s -filetype=obj -o - | llvm-objdump -r -t - | \
