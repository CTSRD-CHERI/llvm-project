...
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+zcheripurecap,+cap-mode,+f,+d < %s | FileCheck %s
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi lp64d -mattr=+zcheripurecap,+zcherihybrid,+f,+d < %s | FileCheck %s --check-prefix=HYBRID
