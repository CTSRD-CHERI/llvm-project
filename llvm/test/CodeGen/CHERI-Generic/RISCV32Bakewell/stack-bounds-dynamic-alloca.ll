...
; RUN: opt -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+zcheripurecap,+cap-mode,+f -cheri-bound-allocas -o - -S %s | FileCheck %s
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+zcheripurecap,+cap-mode,+f -O0 %s -o - | FileCheck %s -check-prefix ASM
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+zcheripurecap,+cap-mode,+f -O2 %s -o - | FileCheck %s -check-prefix ASM-OPT
; reduced C test case:
; __builtin_va_list a;
; char *b;
; void c() {
