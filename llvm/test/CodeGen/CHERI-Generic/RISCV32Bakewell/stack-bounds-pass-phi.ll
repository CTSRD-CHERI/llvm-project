...
; REQUIRES: asserts
; RUN: opt -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+zcheripurecap,+cap-mode,+f -cheri-bound-allocas %s -o - -S -cheri-stack-bounds=if-needed \
; RUN: -cheri-stack-bounds-single-intrinsic-threshold=10 -debug-only=cheri-bound-allocas 2>%t.dbg | FileCheck %s
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+zcheripurecap,+cap-mode,+f -cheri-stack-bounds=if-needed -O2 -cheri-stack-bounds-single-intrinsic-threshold=10 < %s | %cheri_FileCheck %s -check-prefix ASM
...
; ASM-NEXT:    caddi ca0, csp, 12
; ASM-NEXT:    scbndsi ca0, ca0, 4
; ASM-NEXT:    caddi ca1, csp, 4
; ASM-NEXT:  .LBB0_3: # %phi_block
; ASM-NEXT:    scbndsi cs0, ca1, 4
; ASM-NEXT:    call foo
...
; ASM-NEXT:    li a0, 1
; ASM-NEXT:    sw a0, 4(csp)
; ASM-NEXT:    caddi ca0, csp, 4
; ASM-NEXT:    j .LBB1_3
; ASM-NEXT:  .LBB1_2: # %block2
; ASM-NEXT:    li a0, 5
; ASM-NEXT:    sw a0, 0(csp)
; ASM-NEXT:    caddi ca0, csp, 0
