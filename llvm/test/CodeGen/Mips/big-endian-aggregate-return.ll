...
; RUN: opt -march=mips64 -target-abi=n64 -relocation-model=pic -o - -filetype=asm -O3 -S < %s | FileCheck %s
; verify that mips64 little endian still works
; RUN: opt -march=mips64 -data-layout=e-m:e-i8:8:32-i16:16:32-i64:64-n32:64-S128 -target-abi=n64 -relocation-model=pic -o - -filetype=asm -O3 -S < %s | FileCheck %s
; XFAIL: *
