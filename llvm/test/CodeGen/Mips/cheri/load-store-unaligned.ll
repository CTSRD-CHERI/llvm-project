...
; RUNs: FileCheck %s -input-file=%t.dbg -check-prefix=DBG
...
; DBG-NEXT: warning: <unknown>:0:0: found underaligned store of capability type (aligned to 8 bytes instead of {{16|32}}). Will use memcpy() instead of capability load to preserve tags if it is aligned correctly at runtime
; DBG-NEXT: warning: <unknown>:0:0: found underaligned load of capability type (aligned to 8 bytes instead of {{16|32}}). Will use memcpy() instead of capability load to preserve tags if it is aligned correctly at runtime
; DBG-NEXT: alignment_bits,size,kind,source_loc,compiler_pass,details
...
; DBG-NEXT: 4,16,s,"<somewhere in store_of_unaligned_load>","expanding unaligned capability load/store","expanding unaligned capability store stack source"
; DBG-NEXT: 4,16,s,"<somewhere in store_of_unaligned_load>","expanding unaligned capability load/store","expanding unaligned capability store memcpy destination"
; DBG-NEXT: 4,16,s,"<somewhere in store_of_unaligned_load>","expanding unaligned capability load/store","expanding unaligned capability load stack destination"
; DBG-NEXT: 4,16,s,"<somewhere in store_of_unaligned_load>","expanding unaligned capability load/store","expanding unaligned capability load memcpy source"
; DBG-EMPTY:
...
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    cincoffset $c11, $c11, -[[STACKFRAME_SIZE:80|192]]
; CHECK-NEXT:    .cfi_def_cfa_offset [[STACKFRAME_SIZE]]
...
; CHECK-NEXT:    .cfi_offset 91,
; CHECK-NEXT:    .cfi_offset 90,
...
; CHECK-NEXT:    csetbounds $c18, $c4, [[$CAP_SIZE]]
; Bound source for call:
; CHECK-NEXT:    csetbounds $c4, $c3, [[$CAP_SIZE]]
; CHECK-NEXT:    clcbi $c12, %capcall20(memcpy)($c19)
; Stack offset is zero for 128 and 32 for 256
; CHERI256-NEXT: cincoffset $c3, $c11, [[OFFSET_LOAD:32]]
; CHERI256-NEXT: csetbounds $c3, $c3, 32
; CHERI128-NEXT:    csetbounds $c3, $c11, 16
; CHECK-NEXT:    cjalr $c12, $c17
; CHECK-NEXT:    daddiu $4, $zero, [[$CAP_SIZE]]
; CHERI128-NEXT:    clc $c1, $zero, 0($c11)
; CHERI256-NEXT:    clc $c1, $zero, 32($c11)
; CHECK-NEXT:    csc $c1, $zero, [[OFFSET_STORE:16|64]]($c11)
; CHECK-NEXT:    clcbi $c12, %capcall20(memcpy)($c19)
; CHECK-NEXT:    cincoffset $c4, $c11, [[OFFSET_STORE]]
; CHECK-NEXT:    csetbounds $c4, $c4, [[$CAP_SIZE]]
; CHECK-NEXT:    daddiu $4, $zero, [[$CAP_SIZE]]
; CHECK-NEXT:    cjalr $c12, $c17
...
; CHECK-NEXT:    clc $c19, $zero, [[@EXPR STACKFRAME_SIZE - (1 * $CAP_SIZE)]]($c11)
; CHECK-NEXT:    cjr $c17
; CHECK-NEXT:    cincoffset $c11, $c11, [[STACKFRAME_SIZE]]

; HYBRID:      csetbounds	$c17, $c4, [[$CAP_SIZE]]
; HYBRID-NEXT: csetbounds	$c4, $c3, [[$CAP_SIZE]]
; HYBRID-NEXT: daddiu	$16, $sp, 0
; HYBRID-NEXT: cfromddc	$c3, $16
; HYBRID-NEXT: jal	memcpy_c
; HYBRID-NEXT: daddiu	$4, $zero, [[$CAP_SIZE]]
; HYBRID-NEXT: clc	$c1, $16, 0($ddc)
; HYBRID-NEXT: daddiu	$1, $sp, [[$CAP_SIZE]]
; HYBRID-NEXT: csc	$c1, $1, 0($ddc)
; HYBRID-NEXT: cfromddc	$c4, $1
; HYBRID-NEXT: daddiu	$4, $zero, [[$CAP_SIZE]]
; HYBRID-NEXT: jal	memcpy_c
...
entry:
  ...
  %unaligned_load = load i8 addrspace(200)*, i8 addrspace(200)* addrspace(200)* %src_cast, align 8
