...
; RUN: %cheri_opt -opaque-pointers=0 -S -passes=instsimplify %s -o - | FileCheck %s -check-prefix IR
; RUN: %cheri_opt -opaque-pointers=0 -S -passes=instsimplify %s -o - | %cheri_llc -opaque-pointers=0 -O3 -filetype=asm -o - | FileCheck %s -check-prefix OPT
; Test that we can assemble this at -O0 (used to be a problem before csetaddr was added)
; RUN: %cheri_llc -opaque-pointers=0 -O0 %s -filetype=asm -o /dev/null
