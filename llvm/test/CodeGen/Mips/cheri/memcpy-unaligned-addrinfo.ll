...
; RUN: %cheri_purecap_opt -opaque-pointers=0 -passes=sroa -S %s -o - | FileCheck %s -check-prefix SROA
; RUN: %cheri_purecap_opt -opaque-pointers=0 -passes=sroa -S %s -o - | %cheri_purecap_llc -opaque-pointers=0 -O2 - -o - | FileCheck %s -check-prefixes=CHECK,WITH-SROA
; RUN: %cheri_purecap_llc -opaque-pointers=0 -O2 %s -o - | FileCheck %s  -check-prefixes=CHECK,WITHOUT-SROA
...
; SROA-NEXT:    [[DOTFCA_0_INSERT:%.*]] = insertvalue { i8 addrspace(200)* } poison, i8 addrspace(200)* [[RETVAL_SROA_0_0_COPYLOAD]], 0
