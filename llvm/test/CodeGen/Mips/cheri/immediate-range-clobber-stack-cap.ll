; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; RUN: %cheri_purecap_llc -filetype=asm -O0 %s -o - | FileCheck %s
; RUN: %cheri_purecap_llc -filetype=obj -O0 %s -o /dev/null -print-before-all -print-after-all
; XFAIL: *
; We were previously generating a csb with an immediate that was out of range, clobbering other values on the stack!


%"class.Webcore::Settings" = type {}

%"class.Webcore::URL" = type { i8 addrspace(200)*, i8, [300 x i8], i32, i32, i32, i32, i32, i32, i32, i32, i32, i32, [4 x i8] }

declare void @_BAR(%"class.Webcore::Settings" addrspace(200)*, %"class.Webcore::URL" addrspace(200)* dereferenceable(64)) local_unnamed_addr addrspace(200) #0

define hidden void @_ZN19QWebSettingsPrivate5applyEv() local_unnamed_addr addrspace(200) #1 align 2 {
; CHECK-LABEL: _ZN19QWebSettingsPrivate5applyEv:
; CHECK:       # %bb.0:
; CHECK-NEXT:    cincoffset $c11, $c11, -[[STACKFRAME_SIZE:464|928]]
; CHECK-NEXT:    .cfi_def_cfa_offset [[STACKFRAME_SIZE]]
; CHECK-NEXT:    csc $c17, $zero, [[@EXPR 28 * $CAP_SIZE]]($c11)
; CHECK-NEXT:    .cfi_offset 89, -[[@EXPR 1 * $CAP_SIZE]]
; CHECK-NEXT:    lui $1, %hi(%neg(%captab_rel(_ZN19QWebSettingsPrivate5applyEv)))
; CHECK-NEXT:    daddiu $1, $1, %lo(%neg(%captab_rel(_ZN19QWebSettingsPrivate5applyEv)))
; CHECK-NEXT:    cincoffset $c26, $c12, $1
; CHECK-NEXT:    cmove $c12, $c26
; CHECK-NEXT:    cincoffset $c1, $c11, 64
; CHECK-NEXT:    csetbounds $c4, $c1, 368
; CHECK-NEXT:    daddiu $1, $zero, 0
; FIXME: this is sooo wrong
; CHECK-NEXT:    csb $zero, $zero, 444($c11)
; CHECK-NEXT:    addiu $2, $zero, 0
; CHECK-NEXT:    csd $1, $zero, [[@EXPR STACKFRAME_SIZE - 408]]($c11)
; CHECK-NEXT:    csc $c12, $zero, [[@EXPR 2 * $CAP_SIZE]]($c11)
; CHECK-NEXT:    csc $c4, $zero, [[@EXPR 1 * $CAP_SIZE]]($c11)
; CHECK-NEXT:    csw $2, $zero, 12($c11) # 4-byte Folded Spill
; CHECK-NEXT:    bnez $zero, .LBB0_3
; CHECK-NEXT:    nop
; CHECK-NEXT:  # %bb.1:
; CHECK-NEXT:    b .LBB0_2
; CHECK-NEXT:    nop
; CHECK-NEXT:  .LBB0_2: # %lor.lhs.false.i463
; CHECK-NEXT:    b .LBB0_3
; CHECK-NEXT:    nop
; CHECK-NEXT:  .LBB0_3: # %_FOOOO.exit
; CHECK-NEXT:    clc $c1, $zero, [[@EXPR 2 * $CAP_SIZE]]($c11)
; CHECK-NEXT:    clcbi $c12, %capcall20(_BAR)($c1)
; CHECK-NEXT:    # implicit-def: $c3
; CHECK-NEXT:    clc $c4, $zero, [[@EXPR 1 * $CAP_SIZE]]($c11)
; CHECK-NEXT:    cgetnull $c13
; CHECK-NEXT:    cjalr $c12, $c17
; CHECK-NEXT:    nop
  %ref.tmp38 = alloca i8, align 4, addrspace(200)
  %ref.tmp77 = alloca %"class.Webcore::URL", align 16, addrspace(200)
  store i8 0, i8 addrspace(200)* %ref.tmp38, align 4
  br i1 undef, label %_FOOOO.exit, label %lor.lhs.false.i463
lor.lhs.false.i463:                               ; preds = %entry
  br label %_FOOOO.exit
_FOOOO.exit:       ; preds = %lor.lhs.false.i463, %entry
  call void @_BAR(%"class.Webcore::Settings" addrspace(200)* undef, %"class.Webcore::URL" addrspace(200)* nonnull dereferenceable(64) %ref.tmp77) #13
  unreachable
}
