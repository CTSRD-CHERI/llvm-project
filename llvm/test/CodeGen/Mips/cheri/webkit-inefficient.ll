...
; RUN: %cheri128_purecap_llc -opaque-pointers=0 -frame-pointer=all %s -O2 -o - | FileCheck %s '-D#CAP_SIZE=16'
; RUNNOT: %cheri128_purecap_llc -opaque-pointers=0 %s -O2 -o - -filetype=obj | llvm-objdump -d - -no-show-raw-insn -no-leading-addr
; RUNNOT: %cheri128_purecap_llc -opaque-pointers=0 %s -O2 -o - -filetype=obj | llvm-objdump -d - -no-show-raw-insn | FileCheck %s -check-prefix DUMP
; RUN: %cheri128_purecap_llc -opaque-pointers=0 -frame-pointer=all %s -O2 -o - -filetype=obj | llvm-objdump -t - | FileCheck %s -check-prefix SYMBOLS
