; REQUIRES: asserts
; RUN: %cheri_purecap_llc -opaque-pointers=0 -o - -O2 -cheri-cap-table-abi=pcrel %s | %cheri_FileCheck %s -check-prefix CHECK
; RUN: %cheri_purecap_llc -opaque-pointers=0 -o - -O1 -cheri-cap-table-abi=pcrel %s | %cheri_FileCheck %s -check-prefix CHECK
; RUN: %cheri_purecap_llc -opaque-pointers=0 -o /dev/null -O2 -cheri-cap-table-abi=pcrel -debug-only=mips-lower %s 2>&1 | FileCheck %s -check-prefix DEBUG-OUTPUT-CHECK
...
; DEBUG-OUTPUT-CHECK:      Clearing $c13 in call_one_arg_from_many_arg(is varargs: 0) callee = {{t[0-9]+}}: c128 = GlobalAddress<i8 addrspace(200)* (i8 addrspace(200)*) addrspace(200)* @one_arg> 0
; DEBUG-OUTPUT-CHECK-NEXT: Lowering return for function with $c13 live-in: call_one_arg_from_many_arg(is varargs: 0)
; DEBUG-OUTPUT-CHECK-NEXT: Clearing $c13 in call_one_arg_from_variadic_with_va_start(is varargs: 1) callee = {{t[0-9]+}}: c128 = GlobalAddress<i8 addrspace(200)* (i8 addrspace(200)*) addrspace(200)* @one_arg> 0
...
; DEBUG-OUTPUT-CHECK-NEXT: Clearing $c13 in call_one_arg_from_variadic_without_va_start(is varargs: 1) callee = {{t[0-9]+}}: c128 = GlobalAddress<i8 addrspace(200)* (i8 addrspace(200)*) addrspace(200)* @one_arg> 0
...
; DEBUG-OUTPUT-CHECK-NEXT: Clearing $c13 in call_variadic_no_onstack_from_varargs(is varargs: 1) callee = {{t[0-9]+}}: c128 = GlobalAddress<i8 addrspace(200)* (i8 addrspace(200)*, ...) addrspace(200)* @variadic> 0
; DEBUG-OUTPUT-CHECK-NEXT: Lowering return for function with $c13 live-in: call_variadic_no_onstack_from_varargs(is varargs: 1)
; DEBUG-OUTPUT-CHECK-NEXT: Clearing $c13 in call_variadic_no_onstack_from_many_args(is varargs: 0) callee = {{t[0-9]+}}: c128 = GlobalAddress<i8 addrspace(200)* (i8 addrspace(200)*, ...) addrspace(200)* @variadic> 0
