// REQUIRES: asserts
// RUN: %cheri_purecap_clang -xc -O0 -fsanitize-address-use-after-scope -g0 %s -o %S/Inputs/stack-bounds.ll -c -S -emit-llvm -Xclang -disable-O0-optnone -Wno-array-bounds -Wno-return-stack-address
// RUN: %cheri_purecap_opt -cheri-purecap-alloca %t.ll -o /dev/null -S -cheri-stack-bounds-single-intrinsic-call=false \
// RUN:    -cheri-stack-bounds=if-needed -debug-only="cheri-purecap-alloca" 2>&1 | FileCheck -check-prefix DBG %s

// RUN: %cheri_purecap_clang -xc -O1 -fsanitize-address-use-after-scope -g0 %s -o %t-lifetime.ll -c -S -emit-llvm -Xclang -disable-O0-optnone -Wno-array-bounds -Wno-return-stack-address
// RUN: %cheri_purecap_opt -cheri-purecap-alloca %t-lifetime.ll -o /dev/null -S -cheri-stack-bounds-single-intrinsic-call=false \
