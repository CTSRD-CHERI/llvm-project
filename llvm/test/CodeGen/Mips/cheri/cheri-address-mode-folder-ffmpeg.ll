; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; RUN: %cheri_purecap_llc -O3 -verify-machineinstrs %s -o - | FileCheck %s
; ModuleID = '/Users/alex/cheri/llvm/test/CodeGen/Mips/cheri-address-mode-folder-ffmpeg.c'
; This test used to fail the machine verifier
; The instruction after the hoisted global load kills the vreg but LLVM thinkgs it is needed after the basic block
source_filename = "/Users/alex/cheri/llvm/test/CodeGen/Mips/cheri-address-mode-folder-ffmpeg.c"

@c = common addrspace(200) global ptr addrspace(200) null, align 32
@b = common addrspace(200) global i32 0, align 4
@a = common addrspace(200) global i32 0, align 4

; Function Attrs: noinline nounwind
define i32 @fn1(i32 signext %p1) #0 {
; CHECK-LABEL: fn1:
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    cincoffset $c11, $c11, -96
; CHECK-NEXT:    csc $c21, $zero, 80($c11) # 16-byte Folded Spill
; CHECK-NEXT:    csc $c20, $zero, 64($c11) # 16-byte Folded Spill
; CHECK-NEXT:    csc $c19, $zero, 48($c11) # 16-byte Folded Spill
; CHECK-NEXT:    csc $c18, $zero, 32($c11) # 16-byte Folded Spill
; CHECK-NEXT:    csc $c17, $zero, 16($c11) # 16-byte Folded Spill
; CHECK-NEXT:    lui $1, %pcrel_hi(_CHERI_CAPABILITY_TABLE_-8)
; CHECK-NEXT:    daddiu $1, $1, %pcrel_lo(_CHERI_CAPABILITY_TABLE_-4)
; CHECK-NEXT:    cgetpccincoffset $c18, $1
; CHECK-NEXT:    csw $4, $zero, 8($c11)
; CHECK-NEXT:    clcbi $c19, %captab20(c)($c18)
; CHECK-NEXT:    clcbi $c20, %captab20(b)($c18)
; CHECK-NEXT:    clcbi $c21, %captab20(a)($c18)
; CHECK-NEXT:  .LBB0_1: # %while.cond
; CHECK-NEXT:    # =>This Inner Loop Header: Depth=1
; CHECK-NEXT:    clc $c1, $zero, 0($c19)
; CHECK-NEXT:    cbez $c1, .LBB0_7
; CHECK-NEXT:    nop
; CHECK-NEXT:  # %bb.2: # %while.body
; CHECK-NEXT:    # in Loop: Header=BB0_1 Depth=1
; CHECK-NEXT:    clw $1, $zero, 8($c11)
; CHECK-NEXT:    beqz $1, .LBB0_1
; CHECK-NEXT:    nop
; CHECK-NEXT:  # %bb.3: # %land.lhs.true
; CHECK-NEXT:    # in Loop: Header=BB0_1 Depth=1
; CHECK-NEXT:    clw $1, $zero, 0($c20)
; CHECK-NEXT:    clc $c1, $zero, 0($c19)
; CHECK-NEXT:    cincoffset $c2, $cnull, $1
; CHECK-NEXT:    ceq $1, $c1, $c2
; CHECK-NEXT:    bnez $1, .LBB0_1
; CHECK-NEXT:    nop
; CHECK-NEXT:  # %bb.4: # %if.then
; CHECK-NEXT:    # in Loop: Header=BB0_1 Depth=1
; CHECK-NEXT:    clbu $1, $zero, 11($c11)
; CHECK-NEXT:    andi $1, $1, 1
; CHECK-NEXT:    beqz $1, .LBB0_1
; CHECK-NEXT:    nop
; CHECK-NEXT:  # %bb.5: # %land.lhs.true4
; CHECK-NEXT:    # in Loop: Header=BB0_1 Depth=1
; CHECK-NEXT:    clcbi $c12, %capcall20(fn2)($c18)
; CHECK-NEXT:    cjalr $c12, $c17
; CHECK-NEXT:    cgetnull $c13
; CHECK-NEXT:    beqz $2, .LBB0_1
; CHECK-NEXT:    nop
; CHECK-NEXT:  # %bb.6: # %if.then6
; CHECK-NEXT:    # in Loop: Header=BB0_1 Depth=1
; CHECK-NEXT:    clw $1, $zero, 4($c11)
; CHECK-NEXT:    addiu $2, $1, 1
; CHECK-NEXT:    csw $1, $zero, 0($c21)
; CHECK-NEXT:    b .LBB0_1
; CHECK-NEXT:    csw $2, $zero, 4($c11)
; CHECK-NEXT:  .LBB0_7: # %while.end
; CHECK-NEXT:    clw $2, $zero, 12($c11)
; CHECK-NEXT:    clc $c17, $zero, 16($c11) # 16-byte Folded Reload
; CHECK-NEXT:    clc $c18, $zero, 32($c11) # 16-byte Folded Reload
; CHECK-NEXT:    clc $c19, $zero, 48($c11) # 16-byte Folded Reload
; CHECK-NEXT:    clc $c20, $zero, 64($c11) # 16-byte Folded Reload
; CHECK-NEXT:    clc $c21, $zero, 80($c11) # 16-byte Folded Reload
; CHECK-NEXT:    cjr $c17
; CHECK-NEXT:    cincoffset $c11, $c11, 96
entry:
  %retval = alloca i32, align 4, addrspace(200)
  %p1.addr = alloca i32, align 4, addrspace(200)
  %d = alloca i32, align 4, addrspace(200)
  store i32 %p1, ptr addrspace(200) %p1.addr, align 4
  br label %while.cond

while.cond:                                       ; preds = %if.end7, %entry
  %0 = load ptr addrspace(200), ptr addrspace(200) @c, align 32
  %tobool = icmp ne ptr addrspace(200) %0, null
  br i1 %tobool, label %while.body, label %while.end

while.body:                                       ; preds = %while.cond
  %1 = load i32, ptr addrspace(200) %p1.addr, align 4
  %tobool1 = icmp ne i32 %1, 0
  br i1 %tobool1, label %land.lhs.true, label %if.end7

land.lhs.true:                                    ; preds = %while.body
  %2 = load ptr addrspace(200), ptr addrspace(200) @c, align 32
  %3 = load i32, ptr addrspace(200) @b, align 4
  %conv = sext i32 %3 to i64
  %4 = call ptr addrspace(200) @llvm.cheri.cap.offset.set.i64(ptr addrspace(200) null, i64 %conv)
  %5 = bitcast ptr addrspace(200) %4 to ptr addrspace(200)
  %cmp = icmp ne ptr addrspace(200) %2, %5
  br i1 %cmp, label %if.then, label %if.end7

if.then:                                          ; preds = %land.lhs.true
  %6 = load i32, ptr addrspace(200) %p1.addr, align 4
  %and = and i32 %6, 1
  %tobool3 = icmp ne i32 %and, 0
  br i1 %tobool3, label %land.lhs.true4, label %if.end

land.lhs.true4:                                   ; preds = %if.then
  %call = call i32 (...) @fn2()
  %tobool5 = icmp ne i32 %call, 0
  br i1 %tobool5, label %if.then6, label %if.end

if.then6:                                         ; preds = %land.lhs.true4
  %7 = load i32, ptr addrspace(200) %d, align 4
  %inc = add nsw i32 %7, 1
  store i32 %inc, ptr addrspace(200) %d, align 4
  store i32 %7, ptr addrspace(200) @a, align 4
  br label %if.end

if.end:                                           ; preds = %if.then6, %land.lhs.true4, %if.then
  br label %if.end7

if.end7:                                          ; preds = %if.end, %land.lhs.true, %while.body
  br label %while.cond

while.end:                                        ; preds = %while.cond
  %8 = load i32, ptr addrspace(200) %retval, align 4
  ret i32 %8
}

; Function Attrs: nounwind willreturn memory(none)
declare ptr addrspace(200) @llvm.cheri.cap.offset.set.i64(ptr addrspace(200), i64) #1

; Function Attrs: nounwind
declare i32 @fn2(...) #2

; Function Attrs: nounwind willreturn memory(none)
declare ptr addrspace(200) @llvm.cheri.pcc.get() #1

attributes #0 = { noinline nounwind }
attributes #1 = { nounwind willreturn memory(none) }
attributes #2 = { nounwind }

!llvm.module.flags = !{!0, !1}
!llvm.ident = !{!2}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{!"clang version 6.0.0 (https://github.com/llvm-mirror/clang.git 1ea46f5291d82508565b82d4de939241775ab50c) (https://github.com/llvm-mirror/llvm.git f91be1b664a6070160972905806bf7c5ad3f7bee)"}
