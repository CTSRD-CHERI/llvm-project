...
if config.cheri_is_128:
    ...
    config.substitutions.append((r"%cheri128_llc\b", "%cheri_llc"))
    config.substitutions.append((r"%cheri256_llc\b", "llc -mcpu=cheri -cheri-test-mode "))
    config.substitutions.append((r"%cheri_llc\b", "llc -mcpu=cheri128 -mattr=+cheri128"))
    config.substitutions.append((r"%cheri_opt\b", "opt -mcpu=cheri128 -mattr=+cheri128"))
    config.substitutions.append((r"%cheri_clang_cc1\b", "%clang -cc1 -target-cpu cheri128"))
else:
    config.available_features.add("cheri_is_256")
    config.substitutions.append((r"%cheri128_llc\b", "llc -mcpu=cheri128 -cheri-test-mode -mattr=+cheri128"))
    config.substitutions.append((r"%cheri256_llc\b", "%cheri_llc"))
    config.substitutions.append((r"%cheri_llc\b", "llc -mcpu=cheri"))
    config.substitutions.append((r"%cheri_opt\b", "opt -mcpu=cheri"))
