...
; RUN: opt -opaque-pointers=0 -S "-passes=cgscc(inline)" < %s | FileCheck %s --check-prefixes=INLINE-ONLY
; The -O2 pipeline eventually manages to devirtualize the second call but misses the first one:
; RUN: opt -opaque-pointers=0 -S "-passes=default<O2>" < %s | FileCheck %s --check-prefixes=FULL
