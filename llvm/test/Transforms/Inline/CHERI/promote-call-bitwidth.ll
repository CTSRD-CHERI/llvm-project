...
; RUN: opt -opaque-pointers=0 -enable-new-pm=1 -S -passes=inline < %s | FileCheck %s
; RUN: sed -e 's/addrspace(200)/addrspace(0)/g' -e 's/-A200-P200-G200//g' %s | opt -opaque-pointers=0 -enable-new-pm=1 -S -passes=inline| FileCheck %s --check-prefix=HYBRID
