...
; RUN: sed -e 's/addrspace(200)/addrspace(0)/g' -e 's/-A200-P200-G200//g' %s | opt -opaque-pointers=0 -enable-new-pm=1 -S "-passes=instcombine" | FileCheck %s --check-prefix=HYBRID
; RUN: opt -opaque-pointers=0 -enable-new-pm=1 -S "-passes=instcombine" < %s | FileCheck %s --check-prefix=PURECAP
