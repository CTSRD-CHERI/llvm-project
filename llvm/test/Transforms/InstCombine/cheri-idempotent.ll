...
; REQUIRES: asserts
; RUN: opt -opaque-pointers=0 -passes=instcombine -S %s -o - -debug-only=instcombine,instsimplify
; RUN: opt -opaque-pointers=0 -passes=instcombine -S %s -o - | FileCheck %s -check-prefixes CHECK,INSTCOMBINE
; RUN: opt -opaque-pointers=0 -passes=instsimplify -S %s -o - | FileCheck %s -check-prefixes CHECK,INSTSIMPLIFY
