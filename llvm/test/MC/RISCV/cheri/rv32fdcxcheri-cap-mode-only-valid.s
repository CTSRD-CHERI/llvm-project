...
# RUN:     | llvm-objdump --no-print-imm-hex -M no-aliases --mattr=+c,+xcheri,+cap-mode,+f,+d  -d - \
# RUN:     | FileCheck -check-prefixes=CHECK-ASM-AND-OBJ %s

## Floating-point store is only supported in capmode for RV32

# CHECK-ASM-AND-OBJ: {{[[:<:]]}}c.fsd fa0, 40(ca0)
# CHECK-ASM-SAME: encoding: [0x08,0xb5]
c.cfsd fa0, 40(ca0)
# CHECK-ASM-AND-OBJ: {{[[:<:]]}}c.fsd fa0, 40(ca0)
# CHECK-ASM-SAME: encoding: [0x08,0xb5]
c.fsd fa0, 40(ca0)
# CHECK-ASM-AND-OBJ: {{[[:<:]]}}c.fld fa0, 40(ca0)
...
# CHECK-ASM-AND-OBJ: {{[[:<:]]}}c.fsdsp ft1, 40(csp)
# CHECK-ASM-SAME: encoding: [0x06,0xb4]
c.cfsdcsp ft1, 40(csp)
# CHECK-ASM-AND-OBJ: {{[[:<:]]}}c.fsdsp ft1, 40(csp)
# CHECK-ASM-SAME: encoding: [0x06,0xb4]
c.fsdsp ft1, 40(csp)
# CHECK-ASM-AND-OBJ: {{[[:<:]]}}c.fldsp ft1, 40(csp)
