# RUN: llvm-mc -triple riscv64 -mattr=+c,+y,+cap-mode -show-encoding < %s \
# RUN:   | FileCheck -check-prefixes=CHECK-ALIAS %s
# RUN: llvm-mc -triple riscv64 -mattr=+c,+y,+cap-mode -show-encoding \
# RUN:   -riscv-no-aliases < %s | FileCheck -check-prefixes=CHECK-INST %s
# RUN: llvm-mc -triple riscv64 -mattr=+c,+y,+cap-mode -filetype=obj < %s \
# RUN:   | llvm-objdump  --triple=riscv64 --mattr=+c,+y,+cap-mode,-xcheri -d - \
# RUN:   | FileCheck -check-prefixes=CHECK-BYTES,CHECK-ALIAS %s
# RUN: llvm-mc -triple riscv64 -mattr=+c,+y,+cap-mode -filetype=obj < %s \
# RUN:   | llvm-objdump  --triple=riscv64 --mattr=+c,+y,+cap-mode -d -M no-aliases - \
